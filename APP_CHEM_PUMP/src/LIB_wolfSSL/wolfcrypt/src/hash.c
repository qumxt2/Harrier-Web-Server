...
int wc_Hash(enum wc_HashType hash_type, const byte* data,
    wolf_word32 data_len, byte* hash, wolf_word32 hash_len)
{
    int ret = HASH_TYPE_E; /* Default to hash type error */
    wolf_word32 dig_size;
...
int wc_HashFree(wc_HashAlg* hash, enum wc_HashType type)
{
...
#if !defined(NO_MD5)
    int wc_Md5Hash(const byte* data, wolf_word32 len, byte* hash)
    {
        int ret;
...
#if !defined(NO_SHA)
    int wc_ShaHash(const byte* data, wolf_word32 len, byte* hash)
    {
        int ret = 0;
    #ifdef WOLFSSL_SMALL_STACK
        wc_Sha* sha;
    ...
    int wc_Sha224Hash(const byte* data, wolf_word32 len, byte* hash)
    {
        int ret = 0;
    #ifdef WOLFSSL_SMALL_STACK
        wc_Sha224* sha224;
    ...
    int wc_Sha256Hash(const byte* data, wolf_word32 len, byte* hash)
    {
        int ret = 0;
    #ifdef WOLFSSL_SMALL_STACK
        wc_Sha256* sha256;
    ...
    int wc_Sha512Hash(const byte* data, wolf_word32 len, byte* hash)
    {
        int ret = 0;
    #ifdef WOLFSSL_SMALL_STACK
        wc_Sha512* sha512;
    ...
    int wc_Sha384Hash(const byte* data, wolf_word32 len, byte* hash)
    {
        int ret = 0;
    #ifdef WOLFSSL_SMALL_STACK
        wc_Sha384* sha384;
