...
/* fips wrapper calls, user can call direct */
    ...
    (!defined(HAVE_FIPS_VERSION) || (HAVE_FIPS_VERSION < 2))
        ...
        int wc_Sha512Update(wc_Sha512* sha, const byte* data, wolf_word32 len)
        {
        ...
        int wc_Sha384Update(wc_Sha384* sha, const byte* data, wolf_word32 len)
        {
            if (sha == NULL || (data == NULL && len > 0)) {
                return BAD_FUNC_ARG;
            }
...
/* Hardware Acceleration */
    ...
    */
     ...
     */
        ...
        static int Transform_Sha512_AVX1_Len(wc_Sha512 *sha512, wolf_word32 len);
    #endif
    #if defined(HAVE_INTEL_AVX2)
        static int Transform_Sha512_AVX2(wc_Sha512 *sha512);
        static int Transform_Sha512_AVX2_Len(wc_Sha512 *sha512, wolf_word32 len);
    ...
    static int (*Transform_Sha512_Len_p)(wc_Sha512* sha512, wolf_word32 len) = NULL;
...
static WC_INLINE void AddLength(wc_Sha512* sha512, wolf_word32 len)
{
...
static WC_INLINE int Sha512Update(wc_Sha512* sha512, const byte* data, wolf_word32 len)
{
    int ret = 0;
    ...
    if (sha512->buffLen > 0) {
        wolf_word32 add = min(len, WC_SHA512_BLOCK_SIZE - sha512->buffLen);
        if (add > 0) {
    ...
    if (Transform_Sha512_Len_p != NULL) {
        wolf_word32 blocksLen = len & ~(WC_SHA512_BLOCK_SIZE-1);
...
int wc_Sha512Update(wc_Sha512* sha512, const byte* data, wolf_word32 len)
{
...
static int Transform_Sha512_AVX1_RORX_Len(wc_Sha512* sha512, wolf_word32 len)
{
    __asm__ __volatile__ (
...
static int Transform_Sha512_AVX2_RORX_Len(wc_Sha512* sha512, wolf_word32 len)
{
    if ((len & WC_SHA512_BLOCK_SIZE) != 0) {
        XMEMCPY(sha512->buffer, sha512->data, WC_SHA512_BLOCK_SIZE);
...
int wc_Sha384Update(wc_Sha384* sha384, const byte* data, wolf_word32 len)
{
