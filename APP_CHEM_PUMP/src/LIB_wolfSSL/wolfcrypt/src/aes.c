...
/* fips wrapper calls, user can call direct */
    ...
    int wc_AesSetKey(Aes* aes, const byte* key, wolf_word32 len, const byte* iv,
                              int dir)
    {
    ...
    #ifdef HAVE_AES_CBC
        int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
        {
            ...
            int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
            {
                if (aes == NULL || out == NULL || in == NULL
                                            || sz % AES_BLOCK_SIZE != 0) {
                    return BAD_FUNC_ARG;
    ...
    /* AES-CTR */
        ...
        int wc_AesCtrEncrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
        {
            if (aes == NULL || out == NULL || in == NULL) {
                return BAD_FUNC_ARG;
            }
    ...
    /* AES-DIRECT */
        ...
        int wc_AesSetKeyDirect(Aes* aes, const byte* key, wolf_word32 len,
    ...
    /* AES-GCM */
    #ifdef HAVE_AESGCM
        int wc_AesGcmSetKey(Aes* aes, const byte* key, wolf_word32 len)
        {
            if (aes == NULL || !( (len == 16) || (len == 24) || (len == 32)) ) {
                return BAD_FUNC_ARG;
            }
        ...
        int wc_AesGcmEncrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz,
                                      const byte* iv, wolf_word32 ivSz,
                                      byte* authTag, wolf_word32 authTagSz,
                                      const byte* authIn, wolf_word32 authInSz)
        {
        ...
        #ifdef HAVE_AES_DECRYPT
            int wc_AesGcmDecrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz,
                                          const byte* iv, wolf_word32 ivSz,
                                          const byte* authTag, wolf_word32 authTagSz,
                                          const byte* authIn, wolf_word32 authInSz)
            {
        ...
        int wc_GmacSetKey(Gmac* gmac, const byte* key, wolf_word32 len)
        {
        ...
        int wc_GmacUpdate(Gmac* gmac, const byte* iv, wolf_word32 ivSz,
                                      const byte* authIn, wolf_word32 authInSz,
                                      byte* authTag, wolf_word32 authTagSz)
        {
    ...
    /* AES-CCM */
        ...
        defined(HAVE_FIPS_VERSION) && (HAVE_FIPS_VERSION >= 2)
        int wc_AesCcmSetKey(Aes* aes, const byte* key, wolf_word32 keySz)
        {
            return AesCcmSetKey(aes, key, keySz);
        }
        int wc_AesCcmEncrypt(Aes* aes, byte* out, const byte* in, wolf_word32 inSz,
                                      const byte* nonce, wolf_word32 nonceSz,
                                      byte* authTag, wolf_word32 authTagSz,
                                      const byte* authIn, wolf_word32 authInSz)
        {
            ...
            int  wc_AesCcmDecrypt(Aes* aes, byte* out,
                const byte* in, wolf_word32 inSz,
                const byte* nonce, wolf_word32 nonceSz,
                const byte* authTag, wolf_word32 authTagSz,
                const byte* authIn, wolf_word32 authInSz)
            {
...
/* Define AES implementation includes and functions */
    ...
    static wolf_word32 intel_flags = 0;
...
static const wolf_word32 Te[4][256] = {
{
...
#ifdef HAVE_AES_DECRYPT
static const wolf_word32 Td[4][256] = {
{
...
#define GETBYTE(x, y) (wolf_word32)((byte)((x) >> (8 * (y))))
...
static WC_INLINE wolf_word32 PreFetchTe(void)
{
    wolf_word32 x = 0;
    int i,j;
...
{
    wolf_word32 s0, s1, s2, s3;
    wolf_word32 t0, t1, t2, t3;
    wolf_word32 r = aes->rounds >> 1;
    const wolf_word32* rk = aes->key;
...
static WC_INLINE wolf_word32 PreFetchTd(void)
{
    wolf_word32 x = 0;
    int i,j;
...
static WC_INLINE wolf_word32 PreFetchTd4(void)
{
    wolf_word32 x = 0;
    int i;
...
static void wc_AesDecrypt(Aes* aes, const byte* inBlock, byte* outBlock)
{
    ...
    s0 =
        ((wolf_word32)Td4[GETBYTE(t0, 3)] << 24) ^
        ((wolf_word32)Td4[GETBYTE(t3, 2)] << 16) ^
        ((wolf_word32)Td4[GETBYTE(t2, 1)] <<  8) ^
        ((wolf_word32)Td4[GETBYTE(t1, 0)]) ^
        rk[0];
    s1 =
        ((wolf_word32)Td4[GETBYTE(t1, 3)] << 24) ^
        ((wolf_word32)Td4[GETBYTE(t0, 2)] << 16) ^
        ((wolf_word32)Td4[GETBYTE(t3, 1)] <<  8) ^
        ((wolf_word32)Td4[GETBYTE(t2, 0)]) ^
        rk[1];
    s2 =
        ((wolf_word32)Td4[GETBYTE(t2, 3)] << 24) ^
        ((wolf_word32)Td4[GETBYTE(t1, 2)] << 16) ^
        ((wolf_word32)Td4[GETBYTE(t0, 1)] <<  8) ^
        ((wolf_word32)Td4[GETBYTE(t3, 0)]) ^
        rk[2];
    s3 =
        ((wolf_word32)Td4[GETBYTE(t3, 3)] << 24) ^
        ((wolf_word32)Td4[GETBYTE(t2, 2)] << 16) ^
        ((wolf_word32)Td4[GETBYTE(t1, 1)] <<  8) ^
        ((wolf_word32)Td4[GETBYTE(t0, 0)]) ^
        rk[3];
...
/* wc_AesSetKey */
    ...
    int wc_AesSetKey(Aes* aes, const byte* userKey, wolf_word32 keylen,
            const byte* iv, int dir)
    {
        wolf_word32 *rk = aes->key;
    ...
    #if defined(WOLFSSL_AES_DIRECT)
        int wc_AesSetKeyDirect(Aes* aes, const byte* userKey, wolf_word32 keylen,
                            const byte* iv, int dir)
        {
...
#elif defined(FREESCALE_LTC)
    int wc_AesSetKey(Aes* aes, const byte* userKey, wolf_word32 keylen, const byte* iv,
                  int dir)
    {
    ...
    /* Software AES - SetKey */
    static int wc_AesSetKeyLocal(Aes* aes, const byte* userKey, wolf_word32 keylen,
                const byte* iv, int dir)
    {
        wolf_word32 *rk = aes->key;
    #ifdef NEED_AES_TABLES
        ...
        const byte* iv, int dir)
    {
        ...
        const wolf_word32 max_key_len = (AES_MAX_KEY_SIZE / 8);
    #endif

    #ifdef WOLFSSL_IMX6_CAAM_BLOB
        byte   local[32];
        wolf_word32 localSz = 32;
...
/* AES-CBC */
    ...
    {
        int ret = 0;
        wolf_word32 blocks = (sz / AES_BLOCK_SIZE);
        CRYP_HandleTypeDef hcryp;
...
#elif defined(HAVE_COLDFIRE_SEC)
    static int wc_AesCbcCrypt(Aes* aes, byte* po, const byte* pi, wolf_word32 sz,
        wolf_word32 descHeader)
    {
        #ifdef DEBUG_WOLFSSL
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* po, const byte* pi, wolf_word32 sz)
    {
        return (wc_AesCbcCrypt(aes, po, pi, sz, SEC_DESC_AES_CBC_ENCRYPT));
    }

    #ifdef HAVE_AES_DECRYPT
    int wc_AesCbcDecrypt(Aes* aes, byte* po, const byte* pi, wolf_word32 sz)
    {
...
/* AES-CTR */
        ...
        int wc_AesCtrEncryptBlock(Aes* aes, byte* out, const byte* in)
        {
            wolf_word32 tmpIv[AES_BLOCK_SIZE / sizeof(wolf_word32)];
...
static WC_INLINE void IncCtr(byte* ctr, wolf_word32 ctrSz)
{
    int i;
    for (i = ctrSz-1; i >= 0; i--) {
        if (++ctr[i])
...
static WC_INLINE void FlattenSzInBits(byte* buf, wolf_word32 sz)
{
    /* Multiply the sz by 8 */
    wolf_word32 szHi = (sz >> (8*sizeof(sz) - 3));
    sz <<= 3;
...
{                                                          \
    wolf_word32 iv12[4];                                        \
    iv12[0] = *(wolf_word32*)&ivec[0];                          \
    iv12[1] = *(wolf_word32*)&ivec[4];                          \
    iv12[2] = *(wolf_word32*)&ivec[8];                          \
    iv12[3] = 0x01000000;                                  \
...
static void AES_GCM_decrypt(const unsigned char *in,
                           ...
                           int ibytes, wolf_word32 tbytes, const unsigned char* key,
                           int nr, int* res)
{
    int i, j ,k;
    __m128i H, Y, T;
...
void GHASH(Aes* aes, const byte* a, wolf_word32 aSz, const byte* c,
    wolf_word32 cSz, byte* s, wolf_word32 sSz)
{
    byte x[AES_BLOCK_SIZE];
    byte scratch[AES_BLOCK_SIZE];
    wolf_word32 blocks, partial;
    byte* h = aes->H;
...
static void GMULT(wolf_word32* X, wolf_word32* Y)
{
    wolf_word32 Z[4] = {0,0,0,0};
    wolf_word32 V[4];
    int i, j;
...
static WC_INLINE int wc_AesGcmEncrypt_STM32(Aes* aes, byte* out, const byte* in,
                                         wolf_word32 sz, const byte* iv, wolf_word32 ivSz,
                                         byte* authTag, wolf_word32 authTagSz,
                                         const byte* authIn, wolf_word32 authInSz)
{
    int ret;
    ...
    CRYP_HandleTypeDef hcryp;
#else
    wolf_word32 keyCopy[AES_256_KEY_SIZE/sizeof(wolf_word32)];
#endif
    wolf_word32 keySize;
    int status = 0;
    int outPadSz, authPadSz;
    wolf_word32 tag[AES_BLOCK_SIZE/sizeof(wolf_word32)];
    wolf_word32 initialCounter[AES_BLOCK_SIZE/sizeof(wolf_word32)];
    byte* outPadded = NULL;
...
#else /* STD_PERI_LIB */
    ByteReverseWords(keyCopy, (wolf_word32*)aes->key, keySize);
...
#ifdef WOLFSSL_AESNI
int AES_GCM_encrypt_C(Aes* aes, byte* out, const byte* in, wolf_word32 sz,
                      const byte* iv, wolf_word32 ivSz,
                      byte* authTag, wolf_word32 authTagSz,
                      const byte* authIn, wolf_word32 authInSz);
#else
                      ...
                      const byte* authIn, wolf_word32 authInSz)
{
    int ret = 0;
    wolf_word32 blocks = sz / AES_BLOCK_SIZE;
    wolf_word32 partial = sz % AES_BLOCK_SIZE;
    const byte* p = in;
    byte* c = out;
...
#else /* STD_PERI_LIB */
    ByteReverseWords(keyCopy, (wolf_word32*)aes->key, aes->keylen);
...
#ifdef WOLFSSL_AESNI
int AES_GCM_decrypt_C(Aes* aes, byte* out, const byte* in, wolf_word32 sz,
                      const byte* iv, wolf_word32 ivSz,
                      const byte* authTag, wolf_word32 authTagSz,
                      const byte* authIn, wolf_word32 authInSz);
#else
...
int wc_AesGcmSetExtIV(Aes* aes, const byte* iv, wolf_word32 ivSz)
{
    int ret = 0;
...
int wc_AesGcmSetIV(Aes* aes, wolf_word32 ivSz,
                   const byte* ivFixed, wolf_word32 ivFixedSz,
                   WC_RNG* rng)
{
    int ret = 0;
...
int wc_AesGcmEncrypt_ex(Aes* aes, byte* out, const byte* in, wolf_word32 sz,
                        byte* ivOut, wolf_word32 ivOutSz,
                        byte* authTag, wolf_word32 authTagSz,
                        const byte* authIn, wolf_word32 authInSz)
{
    int ret = 0;
...
int wc_Gmac(const byte* key, wolf_word32 keySz, byte* iv, wolf_word32 ivSz,
            const byte* authIn, wolf_word32 authInSz,
            byte* authTag, wolf_word32 authTagSz, WC_RNG* rng)
{
    Aes aes;
    int ret;
...
int wc_GmacVerify(const byte* key, wolf_word32 keySz,
                  const byte* iv, wolf_word32 ivSz,
                  const byte* authIn, wolf_word32 authInSz,
                  const byte* authTag, wolf_word32 authTagSz)
{
...
WOLFSSL_API int wc_GmacSetKey(Gmac* gmac, const byte* key, wolf_word32 len)
{
...
WOLFSSL_API int wc_GmacUpdate(Gmac* gmac, const byte* iv, wolf_word32 ivSz,
                              const byte* authIn, wolf_word32 authInSz,
                              byte* authTag, wolf_word32 authTagSz)
{
...
#ifdef HAVE_AES_DECRYPT
int  wc_AesCcmDecrypt(Aes* aes, byte* out, const byte* in, wolf_word32 inSz,
                   const byte* nonce, wolf_word32 nonceSz,
                   const byte* authTag, wolf_word32 authTagSz,
                   const byte* authIn, wolf_word32 authInSz)
{
...
/* Software CCM */
static void roll_x(Aes* aes, const byte* in, wolf_word32 inSz, byte* out)
{
...
static void roll_auth(Aes* aes, const byte* in, wolf_word32 inSz, byte* out)
{
    wolf_word32 authLenSz;
    wolf_word32 remainder;
...
static WC_INLINE void AesCcmCtrInc(byte* B, wolf_word32 lenSz)
{
    wolf_word32 i;
                   ...
                   const byte* authIn, wolf_word32 authInSz)
{
    ...
    wolf_word32 i;
    byte mask = 0xFF;
    const wolf_word32 wordSz = (wolf_word32)sizeof(wolf_word32);
...
int wc_AesCcmSetNonce(Aes* aes, const byte* nonce, wolf_word32 nonceSz)
{
    int ret = 0;
...
int wc_AesCcmEncrypt_ex(Aes* aes, byte* out, const byte* in, wolf_word32 sz,
                        byte* ivOut, wolf_word32 ivOutSz,
                        byte* authTag, wolf_word32 authTagSz,
                        const byte* authIn, wolf_word32 authInSz)
{
...
int wc_AesGetKeySize(Aes* aes, wolf_word32* keySize)
{
    int ret = 0;
...
/* Software AES - ECB */
int wc_AesEcbEncrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
{
    wolf_word32 blocks = sz / AES_BLOCK_SIZE;
...
int wc_AesEcbDecrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
{
    wolf_word32 blocks = sz / AES_BLOCK_SIZE;
...
int wc_AesCfbEncrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
{
    byte*  tmp = NULL;
    byte*  reg = NULL;
...
int wc_AesCfbDecrypt(Aes* aes, byte* out, const byte* in, wolf_word32 sz)
{
    byte*  tmp;
...
static WC_INLINE void InitKeyWrapCounter(byte* inOutCtr, wolf_word32 value)
{
    int i;
    wolf_word32 bytes;

    bytes = sizeof(wolf_word32);
    for (i = 0; i < (int)sizeof(wolf_word32); i++) {
        inOutCtr[i+sizeof(wolf_word32)] = (value >> ((bytes - 1) * 8)) & 0xFF;
        bytes--;
    }
}
...
int wc_AesKeyWrap(const byte* key, wolf_word32 keySz, const byte* in, wolf_word32 inSz,
                  byte* out, wolf_word32 outSz, const byte* iv)
{
    Aes aes;
    byte* r;
    wolf_word32 i;
...
int wc_AesKeyUnWrap(const byte* key, wolf_word32 keySz, const byte* in, wolf_word32 inSz,
                    byte* out, wolf_word32 outSz, const byte* iv)
{
    Aes aes;
    byte* r;
 ...
 * return 0 on success
 */
int wc_AesXtsSetKey(XtsAes* aes, const byte* key, wolf_word32 len, int dir,
        void* heap, int devId)
{
    wolf_word32 keySz;
    int    ret = 0;
 ...
 * returns 0 on success
 */
int wc_AesXtsDecryptSector(XtsAes* aes, byte* out, const byte* in, wolf_word32 sz,
        word64 sector)
{
    byte* pt;
    byte  i[AES_BLOCK_SIZE];
...
static int _AesXtsHelper(Aes* aes, byte* out, const byte* in, wolf_word32 sz, int dir)
{
    wolf_word32 outSz   = sz;
    wolf_word32 totalSz = (sz / AES_BLOCK_SIZE) * AES_BLOCK_SIZE; /* total bytes */
    byte*  pt      = out;
...
int wc_AesXtsEncrypt(XtsAes* xaes, byte* out, const byte* in, wolf_word32 sz,
        const byte* i, wolf_word32 iSz)
{
    int ret = 0;
    wolf_word32 blocks = (sz / AES_BLOCK_SIZE);
    Aes *aes, *tweak;
...
int wc_AesXtsDecrypt(XtsAes* xaes, byte* out, const byte* in, wolf_word32 sz,
        const byte* i, wolf_word32 iSz)
{
    int ret = 0;
    wolf_word32 blocks = (sz / AES_BLOCK_SIZE);
