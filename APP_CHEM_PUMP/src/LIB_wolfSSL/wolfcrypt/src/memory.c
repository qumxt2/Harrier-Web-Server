...
#endif
{
    ...
    printf("Alloc: %p -> %u at %s:%d\n", res, (wolf_word32)size, func, line);
#else
    (void)func;
    (void)line;
#endif
 ...
 */
static int create_memory_buckets(byte* buffer, wolf_word32 bufSz,
                              wolf_word32 buckSz, wolf_word32 buckNum, wc_Memory** list) {
    wolf_word32 i;
    byte*  pt  = buffer;
    int    ret = 0;
    wolf_word32 memSz = (wolf_word32)sizeof(wc_Memory);
    wolf_word32 padSz = -(int)memSz & (WOLFSSL_STATIC_ALIGN - 1);
...
{
    wolf_word32 wc_MemSz[WOLFMEM_DEF_BUCKETS] = { WOLFMEM_BUCKETS };
    wolf_word32 wc_Dist[WOLFMEM_DEF_BUCKETS]  = { WOLFMEM_DIST };
...
int wolfSSL_load_static_memory(byte* buffer, wolf_word32 sz, int flag,
                                                             WOLFSSL_HEAP* heap)
{
    wolf_word32 ava = sz;
    byte*  pt  = buffer;
...
int wolfSSL_StaticBufferSz(byte* buffer, wolf_word32 sz, int flag)
{
    wolf_word32 bucketSz[WOLFMEM_MAX_BUCKETS] = {WOLFMEM_BUCKETS};
    wolf_word32 distList[WOLFMEM_MAX_BUCKETS] = {WOLFMEM_DIST};
...
#endif
{
    ...
    if (heap == NULL) {
        ...
        if (mem->flag & WOLFMEM_IO_POOL_FIXED &&
            ...
            if (pt == NULL) {
                for (i = 0; i < WOLFMEM_MAX_BUCKETS; i++) {
                    if ((wolf_word32)size < mem->sizeList[i]) {
                        if (mem->ava[i] != NULL) {
                            pt = mem->ava[i];
                            mem->ava[i] = pt->next;
                            break;
...
#endif
{
    ...
    if (ptr) {
        ...
        if (heap == NULL) {
            ...
            WOLFSSL_HEAP_HINT* hint = (WOLFSSL_HEAP_HINT*)heap;
            WOLFSSL_HEAP*      mem  = hint->memory;
            wolf_word32 padSz = -(int)sizeof(wc_Memory) & (WOLFSSL_STATIC_ALIGN - 1);
              ...
              const char* file, unsigned int line)
{
    ...
    if (malloc_function)
        p32 = malloc_function(n + sizeof(wolf_word32) * 4);
    else
        p32 = malloc(n + sizeof(wolf_word32) * 4);

    p32[0] = n;
    p = (void*)(p32 + 4);

    fprintf(stderr, "Alloc: %p -> %u (%d) at %s:%s:%d\n", p, (wolf_word32)n, type,
                                                              func, file, line);
               ...
               const char* file, unsigned int line)
{
    void*   newp = NULL;
    wolf_word32* p32;
    wolf_word32* oldp32 = NULL;
    wolf_word32  oldLen;

    if (p != NULL) {
        oldp32 = (wolf_word32*)p;
        oldp32 -= 4;
        oldLen = oldp32[0];
    }

    if (realloc_function)
        p32 = realloc_function(oldp32, n + sizeof(wolf_word32) * 4);
    else
        p32 = realloc(oldp32, n + sizeof(wolf_word32) * 4);
