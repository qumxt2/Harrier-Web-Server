...
/* fips wrapper calls, user can call direct */
    ...
    int wc_ShaUpdate(wc_Sha* sha, const byte* data, wolf_word32 len)
    {
...
static WC_INLINE void AddLength(wc_Sha* sha, wolf_word32 len)
{
    wolf_word32 tmp = sha->loLen;
...
/* Check if custom wc_Sha transform is used */
    ...
    {
        wolf_word32 W[WC_SHA_BLOCK_SIZE / sizeof(wolf_word32)];

        /* Copy context->state[] to working vars */
        wolf_word32 a = sha->digest[0];
        wolf_word32 b = sha->digest[1];
        wolf_word32 c = sha->digest[2];
        wolf_word32 d = sha->digest[3];
        wolf_word32 e = sha->digest[4];
...
int wc_ShaUpdate(wc_Sha* sha, const byte* data, wolf_word32 len)
{
    ...
    while (len) {
        wolf_word32 add = min(len, WC_SHA_BLOCK_SIZE - sha->buffLen);
...
int wc_ShaFinalRaw(wc_Sha* sha, byte* hash)
{
    ...
    wolf_word32 digest[WC_SHA_DIGEST_SIZE / sizeof(wolf_word32)];
#endif
...
#ifdef LITTLE_ENDIAN_ORDER
    ByteReverseWords((wolf_word32*)digest, (wolf_word32*)sha->digest, WC_SHA_DIGEST_SIZE);
...
int wc_ShaFinal(wc_Sha* sha, byte* hash)
{
    ...
    XMEMCPY(&local[WC_SHA_PAD_SIZE], &sha->hiLen, sizeof(wolf_word32));
    XMEMCPY(&local[WC_SHA_PAD_SIZE + sizeof(wolf_word32)], &sha->loLen, sizeof(wolf_word32));

#if defined(FREESCALE_MMCAU_SHA)
    /* Kinetis requires only these bytes reversed */
    ByteReverseWords(&sha->buffer[WC_SHA_PAD_SIZE/sizeof(wolf_word32)],
                     &sha->buffer[WC_SHA_PAD_SIZE/sizeof(wolf_word32)],
