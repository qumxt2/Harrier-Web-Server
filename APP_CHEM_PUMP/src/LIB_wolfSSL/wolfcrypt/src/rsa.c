...
int  wc_RsaPublicEncrypt(const byte* in, wolf_word32 inLen, byte* out,
                                 wolf_word32 outLen, RsaKey* key, WC_RNG* rng)
{
...
int  wc_RsaPrivateDecryptInline(byte* in, wolf_word32 inLen, byte** out,
                                        RsaKey* key)
{
...
int  wc_RsaPrivateDecrypt(const byte* in, wolf_word32 inLen, byte* out,
                                  wolf_word32 outLen, RsaKey* key)
{
    if (in == NULL || out == NULL || key == NULL) {
        return BAD_FUNC_ARG;
    }
...
int  wc_RsaSSL_Sign(const byte* in, wolf_word32 inLen, byte* out,
                            wolf_word32 outLen, RsaKey* key, WC_RNG* rng)
{
...
int  wc_RsaSSL_VerifyInline(byte* in, wolf_word32 inLen, byte** out, RsaKey* key)
{
    if (in == NULL || out == NULL || key == NULL) {
        return BAD_FUNC_ARG;
    }
...
int  wc_RsaSSL_Verify(const byte* in, wolf_word32 inLen, byte* out,
                              wolf_word32 outLen, RsaKey* key)
{
    if (in == NULL || out == NULL || key == NULL || inLen == 0) {
        return BAD_FUNC_ARG;
...
#ifndef WOLFSSL_RSA_VERIFY_ONLY
int wc_RsaFlattenPublicKey(RsaKey* key, byte* a, wolf_word32* aSz, byte* b,
                           wolf_word32* bSz)
{
...
int wc_InitRsaHw(RsaKey* key)
{
    ...
    wolf_word32 e = 0;     /* RSA public exponent */
    int mSz;
    int eSz;
 ...
 */
static int RsaMGF1(enum wc_HashType hType, byte* seed, wolf_word32 seedSz,
                                        byte* out, wolf_word32 outSz, void* heap)
{
    byte* tmp;
    ...
    if ((seedSz + 4) > sizeof(tmpA) || (wolf_word32)hLen > sizeof(tmpA)) {
        ...
        tmpSz = ((seedSz + 4) > (wolf_word32)hLen)? seedSz + 4: (wolf_word32)hLen;
 ...
 */
static int RsaMGF(int type, byte* seed, wolf_word32 seedSz, byte* out,
                                                    wolf_word32 outSz, void* heap)
{
    int ret;
...
static int RsaPad_OAEP(const byte* input, wolf_word32 inputLen, byte* pkcsBlock,
        wolf_word32 pkcsBlockLen, byte padValue, WC_RNG* rng,
        enum wc_HashType hType, int mgf, byte* optLabel, wolf_word32 labelLen,
        void* heap)
{
    int ret;
    int hLen;
    ...
    if (hLen < 0) {
        ...
        if ((wolf_word32)hLen > sizeof(lHash)) {
     ...
     */
    if ((wolf_word32)(2 * hLen + 2) > pkcsBlockLen) {
    ...
    i = 0;
    idx = hLen + 1;
    while (idx < pkcsBlockLen && (wolf_word32)i < (pkcsBlockLen - hLen -1)) {
    ...
    i = 0;
    while (idx < (wolf_word32)(hLen + 1) && i < hLen) {
 ...
 */
static int RsaPad_PSS(const byte* input, wolf_word32 inputLen, byte* pkcsBlock,
        wolf_word32 pkcsBlockLen, WC_RNG* rng, enum wc_HashType hType, int mgf,
        int saltLen, int bits, void* heap)
{
    ...
    if ((ret = wc_Hash(hType, s, (wolf_word32)(m - s), h, hLen)) != 0)
        return ret;
...
static int RsaPad(const byte* input, wolf_word32 inputLen, byte* pkcsBlock,
                           wolf_word32 pkcsBlockLen, byte padValue, WC_RNG* rng)
{
...
static int wc_RsaPad_ex(const byte* input, wolf_word32 inputLen, byte* pkcsBlock,
    wolf_word32 pkcsBlockLen, byte padValue, WC_RNG* rng, int padType,
    enum wc_HashType hType, int mgf, byte* optLabel, wolf_word32 labelLen,
    int saltLen, int bits, void* heap)
{
    int ret;
    ...
    switch (padType)
    {
        ...
        case WC_RSA_NO_PAD:
             ...
             * the RSA key length */
            if (bits <= 0 || inputLen != ((wolf_word32)bits/WOLFSSL_BIT_SIZE)) {
...
static int RsaUnPad_OAEP(byte *pkcsBlock, unsigned int pkcsBlockLen,
                            ...
                            byte* optLabel, wolf_word32 labelLen, void* heap)
{
    int hLen;
    int ret;
    ...
    hLen = wc_HashGetDigestSize(hType);
    if ((hLen < 0) || (pkcsBlockLen < (2 * (wolf_word32)hLen + 2))) {
        return BAD_FUNC_ARG;
    }
    ...
    for (idx = 0; idx < (wolf_word32)hLen; idx++) {
 ...
 */
static int wc_RsaUnPad_ex(byte* pkcsBlock, wolf_word32 pkcsBlockLen, byte** out,
                          byte padValue, int padType, enum wc_HashType hType,
                          int mgf, byte* optLabel, wolf_word32 labelLen, int saltLen,
                          int bits, void* heap)
{
    int ret;
    ...
    switch (padType) {
        ...
        case WC_RSA_NO_PAD:
            ...
            /* In the case of no padding being used check that input is exactly
             * the RSA key length */
            if (bits <= 0 || pkcsBlockLen != ((wolf_word32)bits/WOLFSSL_BIT_SIZE)) {
                WOLFSSL_MSG("Bad input size");
                ret = RSA_PAD_E;
            }
            else {
 ...
 */
static int wc_RsaFunctionXil(const byte* in, wolf_word32 inLen, byte* out,
                          wolf_word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    int    ret = 0;
    wolf_word32 keyLen, len;
    (void)rng;
...
static int wc_RsaFunctionNonBlock(const byte* in, wolf_word32 inLen, byte* out,
                          wolf_word32* outLen, int type, RsaKey* key)
{
    int    ret = 0;
    wolf_word32 keyLen, len;
...
static int wc_RsaFunctionSync(const byte* in, wolf_word32 inLen, byte* out,
                          wolf_word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
...
#if defined(WOLFSSL_ASYNC_CRYPT) && defined(WC_ASYNC_ENABLE_RSA)
static int wc_RsaFunctionAsync(const byte* in, wolf_word32 inLen, byte* out,
                          wolf_word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    int ret = 0;
 ...
 */
int wc_RsaDirect(byte* in, wolf_word32 inLen, byte* out, wolf_word32* outSz,
...
int wc_RsaFunction(const byte* in, wolf_word32 inLen, byte* out,
                          wolf_word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    int ret = 0;
...
static int RsaPublicEncryptEx(const byte* in, wolf_word32 inLen, byte* out,
                            wolf_word32 outLen, RsaKey* key, int rsa_type,
                            byte pad_value, int pad_type,
                            enum wc_HashType hash, int mgf,
                            byte* label, wolf_word32 labelSz, int saltLen,
                            WC_RNG* rng)
{
    int ret, sz;
    ...
    if (inLen > (wolf_word32)(sz - RSA_MIN_PAD_SZ)) {
#ifdef WC_RSA_NO_PADDING
...
/* Gives the option of choosing padding type
   ...
   saltLen : Length of salt used in PSS
   rng : random number generator */
static int RsaPrivateDecryptEx(byte* in, wolf_word32 inLen, byte* out,
                            wolf_word32 outLen, byte** outPtr, RsaKey* key,
...
int wc_RsaPublicEncrypt(const byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
...
int wc_RsaPublicEncrypt_ex(const byte* in, wolf_word32 inLen, byte* out,
                    wolf_word32 outLen, RsaKey* key, WC_RNG* rng, int type,
...
#ifndef WOLFSSL_RSA_PUBLIC_ONLY
int wc_RsaPrivateDecryptInline(byte* in, wolf_word32 inLen, byte** out, RsaKey* key)
{
    WC_RNG* rng = NULL;
#ifdef WC_RSA_BLINDING
    rng = key->rng;
...
#ifndef WC_NO_RSA_OAEP
int wc_RsaPrivateDecryptInline_ex(byte* in, wolf_word32 inLen, byte** out,
                                  RsaKey* key, int type, enum wc_HashType hash,
                                  int mgf, byte* label, wolf_word32 labelSz)
{
    WC_RNG* rng = NULL;
#ifdef WC_RSA_BLINDING
    rng = key->rng;
...
#if !defined(WC_NO_RSA_OAEP) || defined(WC_RSA_NO_PADDING)
int wc_RsaPrivateDecrypt_ex(const byte* in, wolf_word32 inLen, byte* out,
                            wolf_word32 outLen, RsaKey* key, int type,
                            enum wc_HashType hash, int mgf, byte* label,
                            wolf_word32 labelSz)
{
    WC_RNG* rng = NULL;
...
#ifndef WOLFSSL_RSA_VERIFY_ONLY
int wc_RsaSSL_Verify(const byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
                                                                 RsaKey* key)
{
    WC_RNG* rng;
 ...
 */
int wc_RsaPSS_VerifyInline(byte* in, wolf_word32 inLen, byte** out,
 ...
 * returns the length of the PSS data on success and negative indicates failure.
 */
int wc_RsaPSS_VerifyInline_ex(byte* in, wolf_word32 inLen, byte** out,
 ...
 * returns the length of the PSS data on success and negative indicates failure.
 */
int wc_RsaPSS_Verify(byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
                     enum wc_HashType hash, int mgf, RsaKey* key)
{
 ...
 * returns the length of the PSS data on success and negative indicates failure.
 */
int wc_RsaPSS_Verify_ex(byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
                        enum wc_HashType hash, int mgf, int saltLen,
                        RsaKey* key)
{
    WC_RNG* rng = NULL;
 ...
 */
int wc_RsaPSS_CheckPadding(const byte* in, wolf_word32 inSz, byte* sig,
                           wolf_word32 sigSz, enum wc_HashType hashType)
{
 ...
 * algorithm length and 0 on success.
 */
int wc_RsaPSS_CheckPadding_ex(const byte* in, wolf_word32 inSz, byte* sig,
                              wolf_word32 sigSz, enum wc_HashType hashType,
                              int saltLen, int bits)
{
    ...
    if (in == NULL || sig == NULL ||
                      inSz != (wolf_word32)wc_HashGetDigestSize(hashType))
        ret = BAD_FUNC_ARG;
    ...
    if (ret == 0) {
        ...
        else if (saltLen < -1 || (wolf_word32)saltLen > inSz)
            ret = PSS_SALTLEN_E;
    }
 ...
 * returns the length of the PSS data on success and negative indicates failure.
 */
int wc_RsaPSS_VerifyCheckInline(byte* in, wolf_word32 inLen, byte** out,
                           const byte* digest, wolf_word32 digestLen,
                           enum wc_HashType hash, int mgf, RsaKey* key)
{
 ...
 * returns the length of the PSS data on success and negative indicates failure.
 */
int wc_RsaPSS_VerifyCheck(byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
                          const byte* digest, wolf_word32 digestLen,
                          enum wc_HashType hash, int mgf,
                          RsaKey* key)
{
...
#ifndef WOLFSSL_RSA_PUBLIC_ONLY
int wc_RsaSSL_Sign(const byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
                                                   RsaKey* key, WC_RNG* rng)
{
    return RsaPublicEncryptEx(in, inLen, out, outLen, key,
 ...
 * indicates failure.
 */
int wc_RsaPSS_Sign(const byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
 ...
 * indicates failure.
 */
int wc_RsaPSS_Sign_ex(const byte* in, wolf_word32 inLen, byte* out, wolf_word32 outLen,
...
int wc_RsaFlattenPublicKey(RsaKey* key, byte* e, wolf_word32* eSz, byte* n,
                                                                   wolf_word32* nSz)
{
    int sz, ret;
...
#ifndef WOLFSSL_RSA_VERIFY_ONLY
static int RsaGetValue(mp_int* in, byte* out, wolf_word32* outSz)
{
    wolf_word32 sz;
    int ret = 0;
    ...
    sz = (wolf_word32)mp_unsigned_bin_size(in);
    if (sz > *outSz)
        ret = RSA_BUFFER_E;
...
int wc_RsaExportKey(RsaKey* key,
                    byte* e, wolf_word32* eSz, byte* n, wolf_word32* nSz,
                    byte* d, wolf_word32* dSz, byte* p, wolf_word32* pSz,
                    byte* q, wolf_word32* qSz)
{
    int ret = BAD_FUNC_ARG;
...
int wc_CheckProbablePrime_ex(const byte* pRaw, wolf_word32 pRawSz,
                          const byte* qRaw, wolf_word32 qRawSz,
                          const byte* eRaw, wolf_word32 eRawSz,
...
int wc_CheckProbablePrime(const byte* pRaw, wolf_word32 pRawSz,
                          const byte* qRaw, wolf_word32 qRawSz,
                          const byte* eRaw, wolf_word32 eRawSz,
...
int wc_RsaSetNonBlockTime(RsaKey* key, wolf_word32 maxBlockUs, wolf_word32 cpuMHz)
{
    if (key == NULL || key->nb == NULL) {
        return BAD_FUNC_ARG;
    }
