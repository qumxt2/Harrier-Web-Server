...
/* fips wrapper calls, user can call direct */
    ...
    int wc_Sha256Update(wc_Sha256* sha, const byte* data, wolf_word32 len)
    {
...
/* Hardware Acceleration */
    ...
    */
    ...
    static int Transform_Sha256(wc_Sha256* sha256);
        ...
        static int Transform_Sha256_AVX1_Len(wc_Sha256* sha256, wolf_word32 len);
    #endif
    #if defined(HAVE_INTEL_AVX2)
        static int Transform_Sha256_AVX2(wc_Sha256 *sha256);
        static int Transform_Sha256_AVX2_Len(wc_Sha256* sha256, wolf_word32 len);
        #ifdef HAVE_INTEL_RORX
        static int Transform_Sha256_AVX1_RORX(wc_Sha256 *sha256);
        static int Transform_Sha256_AVX1_RORX_Len(wc_Sha256* sha256, wolf_word32 len);
        static int Transform_Sha256_AVX2_RORX(wc_Sha256 *sha256);
        static int Transform_Sha256_AVX2_RORX_Len(wc_Sha256* sha256, wolf_word32 len);
        #endif
    #endif
    static int (*Transform_Sha256_p)(wc_Sha256* sha256);
                                                       /* = _Transform_Sha256 */
    static int (*Transform_Sha256_Len_p)(wc_Sha256* sha256, wolf_word32 len);
                                                                    /* = NULL */
    static int transform_check = 0;
    static wolf_word32 intel_flags;
    ...
    int wc_Sha256Update(wc_Sha256* sha256, const byte* data, wolf_word32 len)
    {
        int ret = 0;
    ...
    static const ALIGN32 wolf_word32 K[64] = {
    ...
    static int Transform_Sha256(wc_Sha256* sha256)
    {
        wolf_word32 S[8], t0, t1;
        int i;

    #ifdef WOLFSSL_SMALL_STACK_CACHE
        wolf_word32* W = sha256->W;
        if (W == NULL) {
            W = (wolf_word32*)XMALLOC(sizeof(wolf_word32) * WC_SHA256_BLOCK_SIZE, NULL,
                                                           DYNAMIC_TYPE_DIGEST);
            if (W == NULL)
                return MEMORY_E;
            sha256->W = W;
        ...
        if (W == NULL)
            return MEMORY_E;
    #else
        wolf_word32 W[WC_SHA256_BLOCK_SIZE];
    #endif
...
/* End wc_ software implementation */
    ...
    static WC_INLINE void AddLength(wc_Sha256* sha256, wolf_word32 len)
    {
        wolf_word32 tmp = sha256->loLen;
        if ((sha256->loLen += len) < tmp)
            sha256->hiLen++;                       /* carry low to high */
    }

    static WC_INLINE int Sha256Update(wc_Sha256* sha256, const byte* data, wolf_word32 len)
    {
        int ret = 0;
        byte* local;
        ...
        if (sha256->buffLen > 0) {
            wolf_word32 add = min(len, WC_SHA256_BLOCK_SIZE - sha256->buffLen);
        ...
        if (Transform_Sha256_Len_p != NULL) {
            wolf_word32 blocksLen = len & ~(WC_SHA256_BLOCK_SIZE-1);
    ...
    static WC_INLINE int Sha256Final(wc_Sha256* sha256)
    {
        ...
        XMEMCPY(&local[WC_SHA256_PAD_SIZE], &sha256->hiLen, sizeof(wolf_word32));
        XMEMCPY(&local[WC_SHA256_PAD_SIZE + sizeof(wolf_word32)], &sha256->loLen,
                sizeof(wolf_word32));
        ...
        /* Kinetis requires only these bytes reversed */
            ...
            {
                ByteReverseWords(
                    &sha256->buffer[WC_SHA256_PAD_SIZE / sizeof(wolf_word32)],
                    &sha256->buffer[WC_SHA256_PAD_SIZE / sizeof(wolf_word32)],
                    2 * sizeof(wolf_word32));
            }
    #endif
    ...
    int wc_Sha256FinalRaw(wc_Sha256* sha256, byte* hash)
    {
        ...
        wolf_word32 digest[WC_SHA256_DIGEST_SIZE / sizeof(wolf_word32)];
    #endif
    ...
    #ifdef LITTLE_ENDIAN_ORDER
        ByteReverseWords((wolf_word32*)digest, (wolf_word32*)sha256->digest,
...
static const ALIGN32 wolf_word32 K256[128] = {
                                                          ...
                                                          wolf_word32 len)
{
    ...
    int wc_Sha224Update(wc_Sha224* sha224, const byte* data, wolf_word32 len)
    {
        int ret = 0;
