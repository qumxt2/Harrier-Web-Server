...
int wc_GenerateSeed(OS_Seed* os, byte* seed, wolf_word32 sz)
{
...
int wc_RNG_GenerateBlock(WC_RNG* rng, byte* b, wolf_word32 sz)
{
...
int wc_RNG_GenerateByte(WC_RNG* rng, byte* b)
{
    ...
    int wc_RNG_HealthTest(int reseed, const byte* seedA, wolf_word32 seedASz,
                                      const byte* seedB, wolf_word32 seedBSz,
                                      byte* output, wolf_word32 outputSz)
    {
...
#include <wolfssl/wolfcrypt/sha256.h>
    ...
    static int wc_GenerateSeed_IntelRD(OS_Seed* os, byte* output, wolf_word32 sz);
    #endif
    #ifdef HAVE_INTEL_RDRAND
    static int wc_GenerateRand_IntelRD(OS_Seed* os, byte* output, wolf_word32 sz);
    #endif
...
typedef struct DRBG {
    wolf_word32 reseedCtr;
    wolf_word32 lastBlock;
...
static int Hash_df(DRBG* drbg, byte* out, wolf_word32 outSz, byte type,
                                                  const byte* inA, wolf_word32 inASz,
                                                  const byte* inB, wolf_word32 inBSz)
{
    int ret = DRBG_FAILURE;
    byte ctr;
    int i;
    int len;
    wolf_word32 bits = (outSz * 8); /* reverse byte order */
#ifdef WOLFSSL_SMALL_STACK_CACHE
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
static int Hash_DRBG_Reseed(DRBG* drbg, const byte* seed, wolf_word32 seedSz)
{
...
int wc_RNG_DRBG_Reseed(WC_RNG* rng, const byte* seed, wolf_word32 seedSz)
{
...
static WC_INLINE void array_add_one(byte* data, wolf_word32 dataSz)
{
    int i;
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
static int Hash_gen(DRBG* drbg, byte* out, wolf_word32 outSz, const byte* V)
{
    int ret = DRBG_FAILURE;
    byte data[DRBG_SEED_LEN];
    int i;
    int len;
    wolf_word32 checkBlock;
#ifdef WOLFSSL_SMALL_STACK_CACHE
    wc_Sha256* sha = &drbg->sha256;
#else
    wc_Sha256 sha[1];
    ...
    for (i = 0; i < len; i++) {
        ...
        if (ret == 0) {
            XMEMCPY(&checkBlock, digest, sizeof(wolf_word32));
...
static WC_INLINE void array_add(byte* d, wolf_word32 dLen, const byte* s, wolf_word32 sLen)
{
    word16 carry = 0;
...
static int Hash_DRBG_Generate(DRBG* drbg, byte* out, wolf_word32 outSz)
{
    int ret;
#ifdef WOLFSSL_SMALL_STACK_CACHE
    wc_Sha256* sha = &drbg->sha256;
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
static int Hash_DRBG_Instantiate(DRBG* drbg, const byte* seed, wolf_word32 seedSz,
...
int wc_RNG_TestSeed(const byte* seed, wolf_word32 seedSz)
{
    int ret = DRBG_SUCCESS;

    /* Check the seed for duplicate words. */
    wolf_word32 seedIdx = 0;
    wolf_word32 scratchSz = min(SEED_BLOCK_SZ, seedSz - SEED_BLOCK_SZ);
...
static int _InitRng(WC_RNG* rng, byte* nonce, wolf_word32 nonceSz,
                    void* heap, int devId)
{
    int ret = RNG_FAILURE_E;
#ifdef HAVE_HASHDRBG
    wolf_word32 seedSz = SEED_SZ + SEED_BLOCK_SZ;
#endif
...
int wc_InitRngNonce(WC_RNG* rng, byte* nonce, wolf_word32 nonceSz)
{
    return _InitRng(rng, nonce, nonceSz, NULL, INVALID_DEVID);
}


int wc_InitRngNonce_ex(WC_RNG* rng, byte* nonce, wolf_word32 nonceSz,
                       void* heap, int devId)
{
...
int wc_RNG_GenerateBlock(WC_RNG* rng, byte* output, wolf_word32 sz)
{
    int ret;
...
int wc_RNG_HealthTest_ex(int reseed, const byte* nonce, wolf_word32 nonceSz,
                                  const byte* seedA, wolf_word32 seedASz,
                                  const byte* seedB, wolf_word32 seedBSz,
                                  byte* output, wolf_word32 outputSz,
                                  void* heap, int devId)
{
...
/* Begin wc_GenerateSeed Implementations */
     ...
     * int rand_gen_seed(byte* output, wolf_word32 sz);
     * #define CUSTOM_RAND_GENERATE_SEED  rand_gen_seed */

    int wc_GenerateSeed(OS_Seed* os, byte* output, wolf_word32 sz)
    {
     ...
     * Return 0 to indicate success
     * int rand_gen_seed(OS_Seed* os, byte* output, wolf_word32 sz);
...
int wc_GenerateSeed(OS_Seed* os, byte* output, wolf_word32 sz)
{
    ...
    return 0;
        ...
        #endif
//        extern wolf_word32 ReadCoreTimer(void);
        #define PIC32_SEED_COUNT RtosGetRawTickCount
    #endif
        ...
        int wc_GenerateSeed(OS_Seed* os, byte* output, wolf_word32 sz)
        {
            int i;
            byte rnd[8];
            wolf_word32 *rnd32 = (wolf_word32 *)rnd;
            wolf_word32 size = sz;
            byte* op = output;
...
int wc_GenerateSeed(OS_Seed* os, byte* output, wolf_word32 sz)
{
    ...
    int GenerateSeed(OS_Seed* os, byte* output, wolf_word32 sz)
    {
     ...
     * extern int myRngFunc(byte* output, wolf_word32 sz);
     */
