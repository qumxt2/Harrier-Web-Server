...
/* If building for old FIPS. */
    ...
    /* does init */
    int wc_HmacSetKey(Hmac* hmac, int type, const byte* key, wolf_word32 keySz)
    {
    ...
    int wc_HmacUpdate(Hmac* hmac, const byte* in, wolf_word32 sz)
    {
    ...
    #ifdef HAVE_HKDF
        int wc_HKDF(int type, const byte* inKey, wolf_word32 inKeySz,
                    const byte* salt, wolf_word32 saltSz,
                    const byte* info, wolf_word32 infoSz,
...
int wc_HmacSetKey(Hmac* hmac, int type, const byte* key, wolf_word32 length)
{
    byte*  ip;
    byte*  op;
    wolf_word32 i, hmac_block_size = 0;
    int    ret = 0;
    void*  heap = NULL;
...
int wc_HmacUpdate(Hmac* hmac, const byte* msg, wolf_word32 length)
{
    int ret = 0;
...
int wolfSSL_GetHmacMaxSize(void)
{
     ...
     */
    int wc_HKDF_Extract(int type, const byte* salt, wolf_word32 saltSz,
                        const byte* inKey, wolf_word32 inKeySz, byte* out)
    {
     ...
     * returns 0 on success, otherwise failure.
     */
    int wc_HKDF_Expand(int type, const byte* inKey, wolf_word32 inKeySz,
                       const byte* info, wolf_word32 infoSz, byte* out, wolf_word32 outSz)
    {
        byte   tmp[WC_MAX_DIGEST_SIZE];
        Hmac   myHmac;
        int    ret = 0;
        wolf_word32 outIdx = 0;
        wolf_word32 hashSz = wc_HmacSizeByType(type);
        byte   n = 0x1;
