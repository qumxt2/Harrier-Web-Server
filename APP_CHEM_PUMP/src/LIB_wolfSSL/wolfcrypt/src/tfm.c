...
/* math settings check */
wolf_word32 CheckRunTimeSettings(void)
{
    return CTC_SETTINGS;
}
...
wolf_word32 CheckRunTimeFastMath(void)
{
    return FP_SIZE;
}
...
int fp_addmod(fp_int *a, fp_int *b, fp_int *c, fp_int *d)
{
   ...
   * FP_EXPTMOD_NB_CHECKTIME macro to define your own function */
    ...
    static const wolf_word32 exptModNbInst[TFM_EXPTMOD_NB_COUNT] = {
    #ifdef TFM_PPC32
      #ifdef _DEBUG
...
{
#if defined(ALT_ECC_SIZE) || defined(HAVE_WOLF_BIGINT)
  const wolf_word32 maxC = (a->size * sizeof(fp_digit));
#else
  const wolf_word32 maxC = (FP_SIZE * sizeof(fp_digit));
#endif
