...
/* if defined to not use floating point values do not compile in */
    ...
    static wolf_word32 DiscreteLogWorkFactor(wolf_word32 n)
    {
        /* assuming discrete log takes about the same time as factoring */
        if (n < 5)
            return 0;
        else
            return (wolf_word32)(2.4 * XPOW((double)n, 1.0/3.0) *
...
static int GeneratePublicDh(DhKey* key, byte* priv, wolf_word32 privSz,
...
static int wc_DhGenerateKeyPair_Sync(DhKey* key, WC_RNG* rng,
    byte* priv, wolf_word32* privSz, byte* pub, wolf_word32* pubSz)
{
    int ret;
 ...
 */
int wc_DhCheckPubKey_ex(DhKey* key, const byte* pub, wolf_word32 pubSz,
 ...
 *  returns 0 on success or error code
 */
int wc_DhCheckPubKey(DhKey* key, const byte* pub, wolf_word32 pubSz)
{
 ...
 *  returns 0 on success or error code
 */
int wc_DhCheckPrivKey_ex(DhKey* key, const byte* priv, wolf_word32 privSz,
                         const byte* prime, wolf_word32 primeSz)
{
    int ret = 0;
#ifdef WOLFSSL_SMALL_STACK
 ...
 *  returns 0 on success or error code
 */
int wc_DhCheckPrivKey(DhKey* key, const byte* priv, wolf_word32 privSz)
{
 ...
 *  returns 0 on success or error code
 */
int wc_DhCheckKeyPair(DhKey* key, const byte* pub, wolf_word32 pubSz,
...
static int wc_DhAgree_Sync(DhKey* key, byte* agree, wolf_word32* agreeSz,
    const byte* priv, wolf_word32 privSz, const byte* otherPub, wolf_word32 pubSz)
{
    int ret = 0;
#ifdef WOLFSSL_SMALL_STACK
    mp_int* y = NULL;
...
#if defined(WOLFSSL_ASYNC_CRYPT) && defined(WC_ASYNC_ENABLE_DH)
static int wc_DhAgree_Async(DhKey* key, byte* agree, wolf_word32* agreeSz,
    const byte* priv, wolf_word32 privSz, const byte* otherPub, wolf_word32 pubSz)
{
    int ret;
...
int wc_DhAgree(DhKey* key, byte* agree, wolf_word32* agreeSz, const byte* priv,
            wolf_word32 privSz, const byte* otherPub, wolf_word32 pubSz)
{
    int ret = 0;
...
static int _DhSetKey(DhKey* key, const byte* p, wolf_word32 pSz, const byte* g,
                   wolf_word32 gSz, const byte* q, wolf_word32 qSz, int trusted,
                   WC_RNG* rng)
{
    int ret = 0;
    mp_int* keyP = NULL;
...
int wc_DhSetCheckKey(DhKey* key, const byte* p, wolf_word32 pSz, const byte* g,
                   wolf_word32 gSz, const byte* q, wolf_word32 qSz, int trusted,
                   WC_RNG* rng)
{
...
int wc_DhSetKey_ex(DhKey* key, const byte* p, wolf_word32 pSz, const byte* g,
                   wolf_word32 gSz, const byte* q, wolf_word32 qSz)
{
...
int wc_DhSetKey(DhKey* key, const byte* p, wolf_word32 pSz, const byte* g,
                wolf_word32 gSz)
{
 ...
 */
int wc_DhExportParamsRaw(DhKey* dh, byte* p, wolf_word32* pSz,
                         byte* q, wolf_word32* qSz, byte* g, wolf_word32* gSz)
{
    int ret = 0;
    wolf_word32 pLen = 0, qLen = 0, gLen = 0;
