...
typedef struct PacketBuffer {
    wolf_word32  begin;      /* relative sequence begin */
    wolf_word32  end;        /* relative sequence end   */
...
typedef struct NamedKey {
    ...
    wolf_word32           nameSz;                     /* size of server DNS name */
    byte*            key;                        /* DER private key */
    wolf_word32           keySz;                      /* size of DER private key */
...
typedef struct SnifferServer {
    ...
    wolf_word32         server;                       /* netowrk order address */
...
typedef struct FinCaputre {
    wolf_word32 cliFinSeq;               /* client relative sequence FIN  0 is no */
    wolf_word32 srvFinSeq;               /* server relative sequence FIN, 0 is no */
...
typedef struct SnifferSession {
    ...
    wolf_word32         server;          /* server address in network byte order */
    wolf_word32         client;          /* client address in network byte order */
    word16         srvPort;         /* server port */
    word16         cliPort;         /* client port */
    wolf_word32         cliSeqStart;     /* client start sequence */
    wolf_word32         srvSeqStart;     /* server start sequence */
    wolf_word32         cliExpected;     /* client expected sequence (relative) */
    wolf_word32         srvExpected;     /* server expected sequence (relative) */
    ...
    wolf_word32         cliReassemblyMemory; /* client packet memory used */
    wolf_word32         srvReassemblyMemory; /* server packet memory used */
...
static wolf_word32 MissedDataSessions = 0;    /* # of sessions with missed data */
...
typedef struct IpInfo {
    ...
    wolf_word32 src;           /* network order source address */
    wolf_word32 dst;           /* network order destination address */
} IpInfo;
...
typedef struct TcpInfo {
    ...
    int    length;        /* length of this header */
    wolf_word32 sequence;      /* sequence number */
    wolf_word32 ackNumber;     /* ack number */
    byte   fin;           /* FIN set */
    byte   rst;           /* RST set */
    byte   syn;           /* SYN set */
    byte   ack;           /* ACK set */
...
typedef struct TcpPseudoHdr {
    wolf_word32  src;        /* source address */
    wolf_word32  dst;        /* destination address */
...
typedef struct TcpHdr {
    ...
    wolf_word32  sequence;           /* sequence number */
    wolf_word32  ack;                /* acknoledgment number */
...
{
    if (TraceOn) {
        static wolf_word32 packetNumber = 0;
...
static char* IpToS(wolf_word32 addr, char* str)
{
    byte* p = (byte*)&addr;
...
static void TraceSequence(wolf_word32 seq, int len)
{
    if (TraceOn) {
...
/* Show sequence and payload length for Trace */
static void TraceAck(wolf_word32 ack, wolf_word32 expected)
{
    if (TraceOn) {
...
static void TraceRelativeSequence(wolf_word32 expected, wolf_word32 got)
{
    if (TraceOn) {
...
static void TraceServerSyn(wolf_word32 seq)
{
    if (TraceOn) {
...
static void TraceClientSyn(wolf_word32 seq)
{
    if (TraceOn) {
...
static void TraceClientFin(wolf_word32 finSeq, wolf_word32 relSeq)
{
    if (TraceOn) {
...
static void TraceServerFin(wolf_word32 finSeq, wolf_word32 relSeq)
{
    if (TraceOn) {
...
static int IsServerRegistered(wolf_word32 addr)
{
    int ret = 0;     /* false */
...
/* return 1 is true, 0 is false */
static int IsPortRegistered(wolf_word32 port)
{
    int ret = 0;    /* false */
    SnifferServer* sniffer;
...
static wolf_word32 SessionHash(IpInfo* ipInfo, TcpInfo* tcpInfo)
{
    wolf_word32 hash = ipInfo->src * ipInfo->dst;
...
static SnifferSession* GetSnifferSession(IpInfo* ipInfo, TcpInfo* tcpInfo)
{
    ...
    wolf_word32          row = SessionHash(ipInfo, tcpInfo);
...
static int LoadKeyFile(byte** keyBuf, wolf_word32* keyBufSz,
                ...
                const char* password)
{
    ...
    if (typeKey == WOLFSSL_FILETYPE_PEM) {
        ...
        ForceZero(loadBuf, (wolf_word32)fileSz);
        free(loadBuf);

        if (saveBuf) {
            *keyBuf = saveBuf;
            *keyBufSz = (wolf_word32)saveBufSz;
        }
    }
    else {
        *keyBuf = loadBuf;
        *keyBufSz = (wolf_word32)fileSz;
    }
            ...
            const char* keyFile, int typeKey, const char* password, char* error)
{
    ...
    if (name != NULL) {
        ...
        namedKey->nameSz = (wolf_word32)XSTRLEN(name);
                              ...
                              SnifferSession* session, char* error)
{
...
#ifdef HAVE_SNI
    {
        ...
        wolf_word32 nameSz = sizeof(name);
        int ret;
                           ...
                           SnifferSession* session, char* error)
{
    ...
    ret = DoFinished(ssl, input, &inOutIdx, (wolf_word32) size, (wolf_word32) *sslBytes,
                                                                         SNIFF);
...
static int Decrypt(SSL* ssl, byte* output, const byte* input, wolf_word32 sz)
{
    int ret = 0;
    ...
    switch (ssl->specs.bulk_cipher_algorithm) {
        ...
        case wolfssl_aes_gcm:
            if (sz >= (wolf_word32)(AESGCM_EXP_IV_SZ + ssl->specs.aead_mac_size))
            {
...
static const byte* DecryptMessage(SSL* ssl, const byte* input, wolf_word32 sz,
...
static void RemoveSession(SnifferSession* session, IpInfo* ipInfo,
                        TcpInfo* tcpInfo, wolf_word32 rowHint)
{
    SnifferSession* previous = 0;
    SnifferSession* current;
    wolf_word32          row = rowHint;
    int             haveLock = 0;
...
static PacketBuffer* CreateBuffer(wolf_word32* begin, wolf_word32 end, const byte* data,
                                  int* bytesLeft)
{
    PacketBuffer* pb;
...
static int AddToReassembly(byte from, wolf_word32 seq, const byte* sslFrame,
                           int sslBytes, SnifferSession* session, char* error)
{
    ...
    wolf_word32* reassemblyMemory = (from == WOLFSSL_SERVER_END) ?
                  &session->cliReassemblyMemory : &session->srvReassemblyMemory;
    wolf_word32  startSeq = seq;
    wolf_word32  added;
    ...
    if (seq < curr->begin) {
        wolf_word32 end = seq + sslBytes - 1;
    ...
    while (bytesLeft > 0) {
        ...
        if (!curr)
            ...
            added = min((wolf_word32)bytesLeft, curr->begin - seq);
...
static int AddFinCapture(SnifferSession* session, wolf_word32 sequence)
{
    if (session->flags.side == WOLFSSL_SERVER_END) {
...
{
    wolf_word32  seqStart = (session->flags.side == WOLFSSL_SERVER_END) ?
                                     session->cliSeqStart :session->srvSeqStart;
    wolf_word32  real     = tcpInfo->sequence - seqStart;
    wolf_word32* expected = (session->flags.side == WOLFSSL_SERVER_END) ?
    ...
    if (real < *expected) {
        ...
        if (real + *sslBytes > *expected) {
            ...
            *sslBytes -= overlap;
            ...
            if (reassemblyList) {
                wolf_word32 newEnd = *expected + *sslBytes;
                                    ...
                                    const byte** end, char* error)
{
    ...
    byte*        skipPartial = (session->flags.side == WOLFSSL_SERVER_END) ?
                                    &session->flags.srvSkipPartial :
                                    &session->flags.cliSkipPartial;
    wolf_word32* reassemblyMemory = (session->flags.side == WOLFSSL_SERVER_END) ?
        ...
        *expected = curr->end + 1;
        ...
        if (curr->data[0] == application_data &&
            ...
            *sslBytes = curr->end - curr->begin + 1;
            if ( (wolf_word32)*sslBytes > ssl->buffers.inputBuffer.bufferSize) {
    ...
    *skipPartial = 1;
    if (list != NULL)
        *expected = list->begin;
        ...
        wolf_word32     real = tcpInfo->ackNumber - seqStart;
                         ...
                         int* sslBytes, const byte** end, char* error)
{
    ...
    wolf_word32*        length = (session->flags.side == WOLFSSL_SERVER_END) ?
    ...
    wolf_word32*       bufferSize = (session->flags.side == WOLFSSL_SERVER_END) ?
    ...
    while (*front && ((*front)->begin == *expected) ) {
        wolf_word32 room = *bufferSize - *length;
        wolf_word32 packetLen = (*front)->end - (*front)->begin + 1;
