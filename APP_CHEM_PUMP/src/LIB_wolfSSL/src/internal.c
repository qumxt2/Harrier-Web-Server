...
WOLFSSL_CALLBACKS needs LARGE_STATIC_BUFFERS, please add LARGE_STATIC_BUFFERS
    ...
    static int DoHelloVerifyRequest(WOLFSSL* ssl, const byte* input, wolf_word32*,
                                                                        wolf_word32);
    static int DoServerKeyExchange(WOLFSSL* ssl, const byte* input, wolf_word32*,
                                                                        wolf_word32);
    #ifndef NO_CERTS
        static int DoCertificateRequest(WOLFSSL* ssl, const byte* input, wolf_word32*,
                                                                        wolf_word32);
    #endif
    #ifdef HAVE_SESSION_TICKET
        static int DoSessionTicket(WOLFSSL* ssl, const byte* input, wolf_word32*,
                                                                        wolf_word32);
    #endif
#endif
    ...
    static int DoClientKeyExchange(WOLFSSL* ssl, byte* input, wolf_word32*, wolf_word32);
    #if (!defined(NO_RSA) || defined(HAVE_ECC) || defined(HAVE_ED25519)) && \
                                                !defined(WOLFSSL_NO_CLIENT_AUTH)
        static int DoCertificateVerify(WOLFSSL* ssl, byte*, wolf_word32*, wolf_word32);
    #endif
    #ifdef WOLFSSL_DTLS
...
static int SSL_hmac(WOLFSSL* ssl, byte* digest, const byte* in, wolf_word32 sz,
...
static wolf_word32 GetEntropy(unsigned char* out, wolf_word32 num_bytes)
{
    int ret = 0;
...
static int ExportCipherSpecState(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    wolf_word32 idx = 0;
    CipherSpecs* specs;
...
static int ExportKeyState(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    wolf_word32 idx = 0;
    byte   sz;
    Keys* keys;
...
static int ImportCipherSpecState(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    wolf_word32 idx = 0;
    CipherSpecs* specs;
...
static int ImportKeyState(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    wolf_word32 idx = 0;
    byte  sz;
    Keys* keys;
...

    {
        ...
        if (wordCount > WOLFSSL_DTLS_WINDOW_WORDS) {
            ...
            wordAdj = (WOLFSSL_DTLS_WINDOW_WORDS - wordCount) * sizeof(wolf_word32);
        }
...
static int dtls_export_new(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    int idx = 0;
    word16 zero = 0;
 ...
 * On success returns size of buffer used on failure returns a negative value */
static int dtls_export_load(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    int idx = 0;
    Options* options = &ssl->options;
...
static int ExportPeerInfo(WOLFSSL* ssl, byte* exp, wolf_word32 len, byte ver)
{
    int    idx  = 0;
...
static int ImportPeerInfo(WOLFSSL* ssl, byte* buf, wolf_word32 len, byte ver)
{
    word16 idx = 0;
    word16 ipSz;
    word16 fam;
...
int wolfSSL_dtls_export_internal(WOLFSSL* ssl, byte* buf, wolf_word32 sz)
{
    int ret;
    wolf_word32 idx      = 0;
    wolf_word32 totalLen = 0;
...
int wolfSSL_dtls_import_internal(WOLFSSL* ssl, byte* buf, wolf_word32 sz)
{
    wolf_word32 idx    = 0;
    word16 length = 0;
    int version;
...
int RsaSign(WOLFSSL* ssl, const byte* in, wolf_word32 inSz, byte* out,
            wolf_word32* outSz, int sigAlgo, int hashAlgo, RsaKey* key,
...
int RsaVerify(WOLFSSL* ssl, byte* in, wolf_word32 inSz, byte** out, int sigAlgo,
...
int VerifyRsaSign(WOLFSSL* ssl, byte* verifySig, wolf_word32 sigSz,
    const byte* plain, wolf_word32 plainSz, int sigAlgo, int hashAlgo, RsaKey* key,
    DerBuffer* keyBufInfo)
{
...
int RsaDec(WOLFSSL* ssl, byte* in, wolf_word32 inSz, byte** out, wolf_word32* outSz,
...
int RsaEnc(WOLFSSL* ssl, const byte* in, wolf_word32 inSz, byte* out, wolf_word32* outSz,
...
int EccSign(WOLFSSL* ssl, const byte* in, wolf_word32 inSz, byte* out,
    wolf_word32* outSz, ecc_key* key, DerBuffer* keyBufInfo)
{
    int ret;
#ifdef HAVE_PK_CALLBACKS
    const byte* keyBuf = NULL;
...
int EccVerify(WOLFSSL* ssl, const byte* in, wolf_word32 inSz, const byte* out,
    wolf_word32 outSz, ecc_key* key, buffer* keyBufInfo)
{
    int ret;
#ifdef HAVE_PK_CALLBACKS
    const byte* keyBuf = NULL;
...
int EccSharedSecret(WOLFSSL* ssl, ecc_key* priv_key, ecc_key* pub_key,
        byte* pubKeyDer, wolf_word32* pubKeySz, byte* out, wolf_word32* outlen,
        int side)
{
    int ret;
#ifdef HAVE_PK_CALLBACKS
 ...
 */
int Ed25519Sign(WOLFSSL* ssl, const byte* in, wolf_word32 inSz, byte* out,
                wolf_word32* outSz, ed25519_key* key, DerBuffer* keyBufInfo)
{
    int ret;
#ifdef HAVE_PK_CALLBACKS
    const byte* keyBuf = NULL;
 ...
 * returns 0 on success, otherwise the value is an error.
 */
int Ed25519Verify(WOLFSSL* ssl, const byte* in, wolf_word32 inSz, const byte* msg,
                  wolf_word32 msgSz, ed25519_key* key, buffer* keyBufInfo)
{
    int ret;
#ifdef HAVE_PK_CALLBACKS
    const byte* keyBuf = NULL;
...
static int X25519SharedSecret(WOLFSSL* ssl, curve25519_key* priv_key,
        curve25519_key* pub_key, byte* pubKeyDer, wolf_word32* pubKeySz,
        byte* out, wolf_word32* outlen, int side)
{
    int ret;
...
int DhGenKeyPair(WOLFSSL* ssl, DhKey* dhKey,
    byte* priv, wolf_word32* privSz,
    byte* pub, wolf_word32* pubSz)
{
    int ret;
...
int DhAgree(WOLFSSL* ssl, DhKey* dhKey,
    ...
    const byte* otherPub, wolf_word32 otherPubSz,
    byte* agree, wolf_word32* agreeSz)
{
    int ret;
...
#if defined(WOLFSSL_DTLS) || !defined(WOLFSSL_NO_TLS12)
static WC_INLINE void GetSEQIncrement(WOLFSSL* ssl, int verify, wolf_word32 seq[2])
{
    if (verify) {
...
#ifdef WOLFSSL_DTLS
static WC_INLINE void DtlsGetSEQ(WOLFSSL* ssl, int order, wolf_word32 seq[2])
{
    if (order == PREV_ORDER) {
        /* Previous epoch case */
...
DtlsMsg* DtlsMsgNew(wolf_word32 sz, void* heap)
{
    DtlsMsg* msg = NULL;
...
static DtlsFrag* CreateFragment(wolf_word32* begin, wolf_word32 end, const byte* data,
                                byte* buf, wolf_word32* bytesLeft, void* heap)
{
    DtlsFrag* newFrag;
    wolf_word32 added = end - *begin + 1;
...
int DtlsMsgSet(DtlsMsg* msg, wolf_word32 seq, const byte* data, byte type,
                                   wolf_word32 fragOffset, wolf_word32 fragSz, void* heap)
{
    if (msg != NULL && data != NULL && msg->fragSz <= msg->sz &&
        ...
        DtlsFrag* prev = cur;
        DtlsFrag* newFrag;
        wolf_word32 bytesLeft = fragSz; /* could be overlapping fragment */
        wolf_word32 startOffset = fragOffset;
        wolf_word32 added;
        ...
        if (fragOffset < cur->begin) {
            wolf_word32 end = fragOffset + fragSz - 1;
...
DtlsMsg* DtlsMsgFind(DtlsMsg* head, wolf_word32 seq)
{
...
void DtlsMsgStore(WOLFSSL* ssl, wolf_word32 seq, const byte* data,
        wolf_word32 dataSz, byte type, wolf_word32 fragOffset, wolf_word32 fragSz, void* heap)
{
...
int DtlsMsgPoolSave(WOLFSSL* ssl, const byte* data, wolf_word32 dataSz)
{
    DtlsMsg* item;
    int ret = 0;
...
int VerifyForDtlsMsgPoolSend(WOLFSSL* ssl, byte type, wolf_word32 fragOffset)
{
    /**
...
ProtocolVersion MakeDTLSv1_2(void)
{
    ...
    wolf_word32 LowResTimer(void)
    {
        ...
        return (wolf_word32)(count.QuadPart / freq.QuadPart);
    }
    ...
    wolf_word32 LowResTimer(void)
    {
        return (wolf_word32)rtp_get_system_sec();
    }
    ...
    wolf_word32 LowResTimer(void)
    {
        ...
        return (wolf_word32) (ticks / OSCfg_TickRate_Hz);
    }
    ...
    wolf_word32 LowResTimer(void)
    {
        return (wolf_word32) (TickGet() / TICKS_PER_SECOND);
    }
        ...
        wolf_word32 LowResTimer(void)
        {
            return (wolf_word32) (SYS_TMR_TickCountGet() /
        ...
        wolf_word32 LowResTimer(void)
        {
            return (wolf_word32) (SYS_TICK_Get() / SYS_TICK_TicksPerSecondGet());
        }
    ...
    wolf_word32 LowResTimer(void)
    {
        ...
        return (wolf_word32) mqxTime.SECONDS;
    }
    ...
    wolf_word32 LowResTimer(void)
    {
        return (wolf_word32) Seconds_get();
    }
    ...
    wolf_word32 LowResTimer(void)
    {
        ...
        if (con != NULL) {
            if (XRtcPsu_CfgInitialize(&rtc, con, con->BaseAddr)
                    == XST_SUCCESS) {
                return (wolf_word32)XRtcPsu_GetCurrentTime(&rtc);
            }
            else {
    ...
    wolf_word32 LowResTimer(void)
    {
        return (wolf_word32)(uTaskerSystemTick / TICK_RESOLUTION);
    }
...
static void AddRecordHeader(byte* output, wolf_word32 length, byte type, WOLFSSL* ssl)
{
...
static void AddHandShakeHeader(byte* output, wolf_word32 length,
                               wolf_word32 fragOffset, wolf_word32 fragLength,
...
static void AddHeaders(byte* output, wolf_word32 length, byte type, WOLFSSL* ssl)
{
    wolf_word32 lengthAdj = HANDSHAKE_HEADER_SZ;
    wolf_word32 outputAdj = RECORD_HEADER_SZ;
...
#ifndef WOLFSSL_NO_TLS12
#ifndef NO_CERTS
static void AddFragHeaders(byte* output, wolf_word32 fragSz, wolf_word32 fragOffset,
                           wolf_word32 length, byte type, WOLFSSL* ssl)
{
    wolf_word32 lengthAdj = HANDSHAKE_HEADER_SZ;
    wolf_word32 outputAdj = RECORD_HEADER_SZ;
    (void)fragSz;
...
static int wolfSSLReceive(WOLFSSL* ssl, byte* buf, wolf_word32 sz)
{
    int recvd;
...
static int GetRecordHeader(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
...
#ifndef WOLFSSL_NO_TLS12
static int GetHandShakeHeader(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
                              byte *type, wolf_word32 *size, wolf_word32 totalSz)
{
...
static int GetDtlsHandShakeHeader(WOLFSSL* ssl, const byte* input,
                                  wolf_word32* inOutIdx, byte *type, wolf_word32 *size,
                                  wolf_word32 *fragOffset, wolf_word32 *fragSz,
                                  wolf_word32 totalSz)
{
...
static void AddSessionCertToChain(WOLFSSL_X509_CHAIN* chain,
    byte* certBuf, wolf_word32 certSz)
{
...
static int ProcessCSR(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                      wolf_word32 status_length)
{
    int ret = 0;
    OcspRequest* request;
...
int ProcessPeerCerts(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                     wolf_word32 totalSz)
{
    int ret = 0, sigRet = 0;
#ifdef WOLFSSL_ASYNC_CRYPT
...
static int DoCertificate(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                                                                wolf_word32 size)
{
    int ret;
...
static int DoCertificateStatus(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                                                                    wolf_word32 size)
{
    int    ret = 0;
    byte   status_type;
    ...
    switch (status_type) {
     ...
     || defined(HAVE_CERTIFICATE_STATUS_REQUEST_V2)
        ...
        case WOLFSSL_CSR2_OCSP_MULTI: {
            OcspRequest* request;
            wolf_word32 list_length = status_length;
            byte   idx = 0;
...
static int DoHelloRequest(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
                                                    wolf_word32 size, wolf_word32 totalSz)
{
    (void)input;
...
int DoFinished(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx, wolf_word32 size,
                                                      wolf_word32 totalSz, int sniff)
{
    wolf_word32 finishedSz = (ssl->options.tls ? TLS_FINISHED_SZ : FINISHED_SZ);
...
static int DoHandShakeMsgType(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                          byte type, wolf_word32 size, wolf_word32 totalSz)
{
    int ret = 0;
    wolf_word32 expectedIdx;
...
static int DoHandShakeMsg(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                          wolf_word32 totalSz)
{
    int    ret = 0;
    wolf_word32 inputLength;
...
static WC_INLINE int DtlsCheckWindow(WOLFSSL* ssl)
{
    ...
    wolf_word32 cur_lo, next_lo, diff;
    int curLT;
    ...
    if ((next_hi || next_lo > DTLS_SEQ_BITS) &&
        ...
        wolf_word32 idx = diff / DTLS_WORD_BITS;
        wolf_word32 newDiff = diff % DTLS_WORD_BITS;
...
#ifdef WOLFSSL_MULTICAST
static WC_INLINE wolf_word32 UpdateHighwaterMark(wolf_word32 cur, wolf_word32 first,
                                         wolf_word32 second, wolf_word32 max)
{
    wolf_word32 newCur = 0;
...
static WC_INLINE int DtlsUpdateWindow(WOLFSSL* ssl)
{
    ...
    if (curLT) {
        ...
        if (diff >= DTLS_SEQ_BITS)
            ...
            wolf_word32 idx, newDiff, temp, i;
            wolf_word32 oldWindow[WOLFSSL_DTLS_WINDOW_WORDS];
...
static int DoDtlsHandShakeMsg(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                          wolf_word32 totalSz)
{
    byte type;
    wolf_word32 size;
    wolf_word32 fragOffset, fragSz;
    int ret = 0;
                              ...
                              word16 sz)
{
    ...
    int ret       = 0;
    wolf_word32 msgLen = (sz - ssl->specs.aead_mac_size);
    ...
    word16 sz, int asyncOkay)
{
    ...
    wolf_word32 event_flags = WC_ASYNC_FLAG_CALL_AGAIN;
#else
    (void)asyncOkay;
#endif
...
static int SanityCheckCipherText(WOLFSSL* ssl, wolf_word32 encryptSz)
{
#ifdef HAVE_TRUNCATED_HMAC
    wolf_word32 minLength = ssl->truncated_hmac ? (byte)TRUNCATED_HMAC_SZ
                                           : ssl->specs.hash_size;
#else
    wolf_word32 minLength = ssl->specs.hash_size; /* covers stream */
#endif
...
int DoApplicationData(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx)
{
    wolf_word32 msgSz   = ssl->keys.encryptSz;
    wolf_word32 idx     = *inOutIdx;
    int    dataSz;
    int    ivExtra = 0;
...
static int DoAlert(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx, int* type,
                   wolf_word32 totalSz)
{
    byte level;
    byte code;
...
static int GetInputData(WOLFSSL *ssl, wolf_word32 size)
{
    int in;
    int inSz;
    int maxLength;
...
static WC_INLINE int VerifyMac(WOLFSSL* ssl, const byte* input, wolf_word32 msgSz,
                            int content, wolf_word32* padSz)
{
#if !defined(WOLFSSL_NO_TLS12) && !defined(WOLFSSL_AEAD_ONLY)
    int    ivExtra = 0;
    int    ret;
    wolf_word32 pad     = 0;
    wolf_word32 padByte = 0;
#ifdef HAVE_TRUNCATED_HMAC
    wolf_word32 digestSz = ssl->truncated_hmac ? (byte)TRUNCATED_HMAC_SZ
                                          : ssl->specs.hash_size;
#else
    wolf_word32 digestSz = ssl->specs.hash_size;
#endif
    byte   verify[WC_MAX_DIGEST_SIZE];
...
int ProcessReply(WOLFSSL* ssl)
{
    ...
    #else
                    if (IsAtLeastTLSv1_3(ssl->version)) {
                        wolf_word32 i = ssl->buffers.inputBuffer.idx;
                        if (ssl->curSize != 1 ||
                    ...
                    int padLen, int content, int verify)
{
    ...
    wolf_word32 digestSz = ssl->specs.hash_size;            /* actual sizes */
    wolf_word32 padSz    = ssl->specs.pad_size;
    int    ret      = 0;
...
typedef struct BuildMsgArgs {
    wolf_word32 digestSz;
    wolf_word32 sz;
    wolf_word32 pad;
    wolf_word32 idx;
    wolf_word32 headerSz;
    word16 size;
    wolf_word32 ivSz;      /* TLSv1.1  IV */
    byte*  iv;
} BuildMsgArgs;
             ...
             int inSz, int type, int hashOutput, int sizeOnly, int asyncOkay)
{
    ...
    switch (ssl->options.buildMsgState) {
        ...
        #ifndef WOLFSSL_AEAD_ONLY
            if (ssl->specs.cipher_type == block) {
                wolf_word32 blockSz = ssl->specs.block_size;
                if (ssl->options.tls1_1) {
                    args->ivSz = blockSz;
                    args->sz  += args->ivSz;
        ...
        #ifndef WOLFSSL_AEAD_ONLY
            if (ssl->specs.cipher_type == block) {
                wolf_word32 tmpIdx = args->idx + args->digestSz;
                wolf_word32 i;
...
static int CreateOcspRequest(WOLFSSL* ssl, OcspRequest* request,
                             DecodedCert* cert, byte* certData, wolf_word32 length)
{
    int ret;
...
{
    int    ret = 0;
    wolf_word32 certSz, certChainSz, headerSz, listSz, payloadSz;
    wolf_word32 length, maxFragment;
    ...
    while (length > 0 && ret == 0) {
        byte*  output = NULL;
        wolf_word32 fragSz = 0;
        wolf_word32 i = RECORD_HEADER_SZ;
        ...
        if (certSz && ssl->fragOffset < certSz) {
            wolf_word32 copySz = min(certSz - ssl->fragOffset, fragSz);
            XMEMCPY(output + i,
        ...
        if (certChainSz && fragSz) {
            wolf_word32 copySz = min(certChainSz + certSz - ssl->fragOffset, fragSz);
            XMEMCPY(output + i,
...
int SendCertificateRequest(WOLFSSL* ssl)
{
    ...
    int    ret;
    int    sendSz;
    wolf_word32 i = RECORD_HEADER_SZ + HANDSHAKE_HEADER_SZ;
    wolf_word32 dnLen = 0;
#if defined(OPENSSL_ALL) || defined(WOLFSSL_NGINX)
...
{
    byte*  output  = NULL;
    wolf_word32 idx     = RECORD_HEADER_SZ + HANDSHAKE_HEADER_SZ;
    wolf_word32 length  = ENUM_LEN;
    int    sendSz  = 0;
    int    ret     = 0;
    int    i       = 0;
...
int SetCipherList(WOLFSSL_CTX* ctx, Suites* suites, const char* list)
{
    ...
    do {
        ...
        next   = XSTRSTR(next, ":");
        length = min(sizeof(name), !next ? (wolf_word32)XSTRLEN(current) /* last */
                                         : (wolf_word32)(next - current));
...
/* client only parts */
    ...
    {
        byte              *output;
        wolf_word32             length, idx = RECORD_HEADER_SZ + HANDSHAKE_HEADER_SZ;
        int                sendSz;
    ...
    static int DoHelloVerifyRequest(WOLFSSL* ssl, const byte* input,
                                    wolf_word32* inOutIdx, wolf_word32 size)
    {
        ProtocolVersion pv;
        byte            cookieSz;
        wolf_word32          begin = *inOutIdx;
    ...
    int DoServerHello(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
                      wolf_word32 helloSz)
    {
 ...
 */
static int NtruSecretEncrypt(QSHKey* key, byte* bufIn, wolf_word32 inSz,
...
static int NtruSecretDecrypt(QSHKey* key, byte* bufIn, wolf_word32 inSz,
        byte* bufOut, word16* outSz)
{
    int    ret;
    DRBG_HANDLE drbg;
...
static int QSH_Encrypt(QSHKey* key, byte* in, wolf_word32 szIn,
                                                       byte* out, wolf_word32* szOut)
{
    int ret = 0;
    word16 size = *szOut;
...
int QSH_Decrypt(QSHKey* key, byte* in, wolf_word32 szIn, byte* out, word16* szOut)
{
    int ret = 0;
    word16 size = *szOut;
 ...
 */
static wolf_word32 QSH_MaxSecret(QSHKey* key)
{
    int ret = 0;
#ifdef HAVE_NTRU
    byte isNtru = 0;
...
static wolf_word32 QSH_KeyGetSize(WOLFSSL* ssl)
{
    wolf_word32 sz = 0;
    QSHKey* current = ssl->peerQSHKey;
...
/* handle QSH key Exchange
   return 0 on success
 */
static wolf_word32 QSH_KeyExchangeWrite(WOLFSSL* ssl, byte isServer)
{
    int ret = 0;
        ...
        case TLS_ASYNC_BUILD:
        {
            ...
            switch(ssl->specs.kea)
            {
                ...
                case psk_kea:
                {
                    ...
                    args->encSz = (wolf_word32)XSTRLEN(ssl->arrays->client_identity);
                ...
                case dhe_psk_kea:
                {
                    ...
                    ssl->arrays->client_identity[MAX_PSK_ID_LEN] = '\0'; /* null term */
                    esSz = (wolf_word32)XSTRLEN(ssl->arrays->client_identity);
...
int SendCertificateVerify(WOLFSSL* ssl)
{
    ...
    switch(ssl->options.asyncState)
    {
        ...
        case TLS_ASYNC_FINALIZE:
        {
            ...
            AddHeaders(args->output, (wolf_word32)args->length + args->extraSz +
                                        VERIFY_HEADER, certificate_verify, ssl);

            args->sendSz = RECORD_HEADER_SZ + HANDSHAKE_HEADER_SZ +
                           (wolf_word32)args->length + args->extraSz + VERIFY_HEADER;
...
#ifdef HAVE_SESSION_TICKET
int SetTicket(WOLFSSL* ssl, const byte* ticket, wolf_word32 length)
{
...
static int DoSessionTicket(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
    wolf_word32 size)
{
    wolf_word32 begin = *inOutIdx;
    wolf_word32 lifetime;
    word16 length;
    int    ret;
    ...
    *inOutIdx += length;
    ...
    typedef struct SskeArgs {
        ...
        byte*  input;
        wolf_word32 idx;
        wolf_word32 tmpSigSz;
        wolf_word32 length;
        wolf_word32 sigSz;
    #if defined(HAVE_ECC) || defined(HAVE_ED25519) || \
                                           (!defined(NO_DH) && !defined(NO_RSA))
        wolf_word32 sigDataSz;
    #endif
    #if defined(HAVE_ECC) || defined(HAVE_CURVE25519)
        wolf_word32 exportSz;
    #endif
    #ifdef HAVE_QSH
        wolf_word32 qshSz;
    #endif
    ...
    int SendServerKeyExchange(WOLFSSL* ssl)
    {
        ...
        switch(ssl->options.asyncState)
        {
            ...
            case TLS_ASYNC_BUILD:
            {
                                                       ...
                                                       defined(HAVE_CURVE25519))
                wolf_word32 preSigSz, preSigIdx;
            #endif
                ...
                switch(ssl->specs.kea)
                {
                    ...
                    case psk_kea:
                    {
                        ...
                        /* include size part */
                        args->length = (wolf_word32)XSTRLEN(ssl->arrays->server_hint);
                    ...
                    case dhe_psk_kea:
                    {
                        ...
                        /* include size part */
                        hintLen = (wolf_word32)XSTRLEN(ssl->arrays->server_hint);
                    ...
                    case ecc_diffie_hellman_kea:
                    {
                        ...
                        if (ssl->buffers.key == NULL) {
                            ...
                            case rsa_sa_algo:
                            {
                                ...
                                args->tmpSigSz = (wolf_word32)keySz;
                                if (keySz < ssl->options.minRsaKeySz) {
                    ...
                    case diffie_hellman_kea:
                    {
                        ...
                        if (!ssl->options.usingAnon_cipher) {
                            ...
                            #ifdef HAVE_PK_CALLBACKS
                                if (wolfSSL_CTX_IsPrivatePkSet(ssl->ctx))
                                    keySz = (wolf_word32)GetPrivateKeySigSize(ssl);
                                else
                            #endif
                                    ERROR_OUT(NO_PRIVATE_KEY, exit_sske);
                            }
    ...
    int ProcessOldClientHello(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
                              wolf_word32 inSz, word16 sz)
    {
        wolf_word32          idx = *inOutIdx;
        word16          sessionSz;
        word16          randomSz;
    ...
    int DoClientHello(WOLFSSL* ssl, const byte* input, wolf_word32* inOutIdx,
                             wolf_word32 helloSz)
    {
        byte            b;
...
#define WOLFSSL_TICKET_ENC_SZ (SESSION_TICKET_LEN - WOLFSSL_TICKET_FIXED_SZ)
    ...
    int DoClientTicket(WOLFSSL* ssl, const byte* input, wolf_word32 len)
    {
        ExternalTicket* et;
        InternalTicket* it;
        int             ret;
        ...
        if (len > SESSION_TICKET_LEN ||
             len < (wolf_word32)(sizeof(InternalTicket) + WOLFSSL_TICKET_FIXED_SZ)) {
    ...
    int SendTicket(WOLFSSL* ssl)
    {
        ...
        int                ret;
        int                sendSz;
        wolf_word32             length = SESSION_HINT_SZ + LENGTH_SZ;
        wolf_word32             idx    = RECORD_HEADER_SZ + HANDSHAKE_HEADER_SZ;
    ...
    static int DoClientKeyExchange(WOLFSSL* ssl, byte* input, wolf_word32* inOutIdx,
                                                                    wolf_word32 size)
    {
        int ret;
    #ifdef WOLFSSL_ASYNC_CRYPT
                ...
                switch (ssl->specs.kea) {
                    ...
                    case rsa_kea:
                    {
                        ...
                        if (ssl->options.tls) {
                            ...
                            if ((wolf_word32)check != args->length) {
...
int wolfSSL_AsyncInit(WOLFSSL* ssl, WC_ASYNC_DEV* asyncDev, wolf_word32 flags)
{
    int ret;
    WOLF_EVENT* event;
