...
static int p_hash(byte* result, wolf_word32 resLen, const byte* secret,
                  wolf_word32 secLen, const byte* seed, wolf_word32 seedLen, int hash,
                  void* heap, int devId)
{
    wolf_word32 len = P_HASH_MAX_SIZE;
    wolf_word32 times;
    wolf_word32 lastLen;
    wolf_word32 lastTime;
    wolf_word32 i;
    wolf_word32 idx = 0;
    int    ret = 0;
...
static WC_INLINE void get_xor(byte *digest, wolf_word32 digLen, byte* md5, byte* sha)
{
    wolf_word32 i;
...
static int doPRF(byte* digest, wolf_word32 digLen, const byte* secret,wolf_word32 secLen,
                 const byte* label, wolf_word32 labLen, const byte* seed,
                 wolf_word32 seedLen, void* heap, int devId)
{
    int    ret  = 0;
    wolf_word32 half = (secLen + 1) / 2;
...
/* Wrapper to call straight thru to p_hash in TSL 1.2 cases to remove stack
   use */
static int PRF(byte* digest, wolf_word32 digLen, const byte* secret, wolf_word32 secLen,
            const byte* label, wolf_word32 labLen, const byte* seed, wolf_word32 seedLen,
...
int BuildTlsHandshakeHash(WOLFSSL* ssl, byte* hash, wolf_word32* hashLen)
{
    int ret = 0;
    wolf_word32 hashSz = FINISHED_SZ;
...
static int _DeriveTlsKeys(byte* key_dig, wolf_word32 key_dig_len,
                         const byte* ms, wolf_word32 msLen,
...
int wolfSSL_DeriveTlsKeys(byte* key_dig, wolf_word32 key_dig_len,
                         const byte* ms, wolf_word32 msLen,
                         const byte* sr, const byte* cr,
                         int tls1_2, int hash_type)
{
...
static int _MakeTlsMasterSecret(byte* ms, wolf_word32 msLen,
                               const byte* pms, wolf_word32 pmsLen,
...
/* External facing wrapper so user can call as well, 0 on success */
int wolfSSL_MakeTlsMasterSecret(byte* ms, wolf_word32 msLen,
                               const byte* pms, wolf_word32 pmsLen,
                               const byte* cr, const byte* sr,
                               int tls1_2, int hash_type)
{
...
static int _MakeTlsExtendedMasterSecret(byte* ms, wolf_word32 msLen,
                                        const byte* pms, wolf_word32 pmsLen,
                                        const byte* sHash, wolf_word32 sHashLen,
                                        int tls1_2, int hash_type,
                                        void* heap, int devId)
{
...
/* External facing wrapper so user can call as well, 0 on success */
int wolfSSL_MakeTlsExtendedMasterSecret(byte* ms, wolf_word32 msLen,
                                        const byte* pms, wolf_word32 pmsLen,
                                        const byte* sHash, wolf_word32 sHashLen,
                                        int tls1_2, int hash_type)
{
                                                              ...
                                                              const char* label)
{
    ...
    ret = PRF((byte*)msk, len, ssl->arrays->masterSecret, SECRET_LEN,
              (const byte *)label, (wolf_word32)XSTRLEN(label), seed, SEED_LEN,
              IsAtLeastTLSv1_2(ssl), ssl->specs.mac_algorithm,
              ssl->heap, ssl->devId);
...
int wolfSSL_SetTlsHmacInner(WOLFSSL* ssl, byte* inner, wolf_word32 sz, int content,
                           int verify)
{
 ...
 */
static int Hmac_HashUpdate(Hmac* hmac, const byte* data, wolf_word32 sz)
{
                               ...
                               wolf_word32 sz, byte* header)
{
    ...
    c32toa(realLen >> ((sizeof(wolf_word32) * 8) - 3), lenBytes);
    c32toa(realLen << 3, lenBytes + sizeof(wolf_word32));
    ...
    for (i = safeBlocks; i < blocks; i++) {
        ...
        for (j = 0; j < blockSz; j++, k++) {
            ...
            b &= (unsigned char)~(wolf_word32)pastEoc;
            b &= ((unsigned char)~(wolf_word32)isOutBlock) | isEocBlock;
                            ...
                            wolf_word32 sz, byte* header)
{
    ...
    int        ret;
    wolf_word32     msgSz, blockSz, macSz, padSz, maxSz, realSz;
    wolf_word32     currSz, offset;
...
int TLS_hmac(WOLFSSL* ssl, byte* digest, const byte* in, wolf_word32 sz, int padSz,
...
int TLSX_SNI_GetFromBuffer(const byte* clientHello, wolf_word32 helloSz,
                           byte type, byte* sni, wolf_word32* inOutSz)
{
    wolf_word32 offset = 0;
    wolf_word32 len32 = 0;
    word16 len16 = 0;
...
int TLSX_ValidateSupportedCurves(WOLFSSL* ssl, byte first, byte second) {
    ...
    wolf_word32         oid       = 0;
    wolf_word32         pkOid     = 0;
    wolf_word32         defOid    = 0;
    wolf_word32         defSz     = 80; /* Maximum known curve size is 66. */
    wolf_word32         nextOid   = 0;
    wolf_word32         nextSz    = 80; /* Maximum known curve size is 66. */
    wolf_word32         currOid   = ssl->ecdhCurveOID;
    int            ephmSuite = 0;
...
#ifdef HAVE_CURVE25519
    byte*           keyData = NULL;
    wolf_word32          dataSize = CURVE25519_KEYSIZE;
    curve25519_key* key;
...
static int TLSX_KeyShare_ProcessDh(WOLFSSL* ssl, KeyShareEntry* keyShareEntry)
{
    ...
    if (params->p_len > ssl->arrays->preMasterSz) {
        wolf_word32 diff = params->p_len - ssl->arrays->preMasterSz;
 ...
 * returns 0 on success and other values indicate failure.
 */
int TLSX_PreSharedKey_Use(WOLFSSL* ssl, byte* identity, word16 len, wolf_word32 age,
 ...
 * returns the number of bytes written into the buffer.
 */
static word16 TLSX_EarlyData_Write(wolf_word32 max, byte* output, byte msgType)
{
    if (msgType == client_hello || msgType == encrypted_extensions) {
        return 0;
    }
 ...
 * returns 0 on success and other values indicate failure.
 */
int TLSX_EarlyData_Use(WOLFSSL* ssl, wolf_word32 max)
{
    int   ret = 0;
    TLSX* extension;
