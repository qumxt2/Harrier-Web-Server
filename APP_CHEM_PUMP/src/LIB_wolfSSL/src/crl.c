    ...
    while (crle) {
        if (XMEMCMP(crle->issuerHash, cert->issuerHash, CRL_DIGEST_SIZE) == 0) {
            ...
            if (crle->verified == 0) {
                ...
                byte* tbs = NULL;
                wolf_word32 tbsSz = crle->tbsSz;
                byte* sig = NULL;
                wolf_word32 sigSz = crle->signatureSz;
                wolf_word32 sigOID = crle->signatureOID;
                SignatureCtx sigCtx;
                  ...
                  int noVerify)
{
    ...
    ret = ParseCRL(dcrl, myBuffer, (wolf_word32)sz, crl->cm);
...
int LoadCRL(WOLFSSL_CRL* crl, const char* path, int type, int monitor)
{
    ...
    if (monitor & WOLFSSL_CRL_MONITOR) {
        ...
        pathLen = (wolf_word32)XSTRLEN(path);
