...
struct DecodedCert {
    ...
    wolf_word32  pubKeySize;
    int     pubKeyStored;
    wolf_word32  certBegin;               /* offset to start of cert          */
    wolf_word32  sigIndex;                /* offset to start of signature     */
    wolf_word32  sigLength;               /* length of signature              */
    wolf_word32  signatureOID;            /* sum of algorithm object id       */
    wolf_word32  keyOID;                  /* sum of key algo  object id       */
    ...
    wolf_word32  srcIdx;                  /* current offset into buffer       */
    wolf_word32  maxIdx;                  /* max offset based on init size    */
    void*   heap;                    /* for user memory overrides        */
    byte    serial[EXTERNAL_SERIAL_SIZE];  /* raw serial number          */
    int     serialSz;                /* raw serial bytes stored */
    const byte* extensions;          /* not owned, points into raw cert  */
    int     extensionsSz;            /* length of cert extensions */
    wolf_word32  extensionsIdx;           /* if want to go back and parse later */
    ...
    wolf_word32  extExtKeyUsageSz;
    wolf_word32  extExtKeyUsageCount;
    const byte* extAuthKeyIdSrc;
    wolf_word32  extAuthKeyIdSz;
    const byte* extSubjKeyIdSrc;
    wolf_word32  extSubjKeyIdSz;
#endif

#if defined(HAVE_ECC) || defined(HAVE_ED25519)
    wolf_word32  pkCurveOID;           /* Public Key's curve OID */
#endif /* HAVE_ECC */
...
WOLFSSL_ASN_API int wc_BerToDer(const byte* ber, wolf_word32 berSz, byte* der,
                                wolf_word32* derSz);
...
#endif /* IGNORE_NAME_CONSTRAINTS */
WOLFSSL_ASN_API void InitDecodedCert(DecodedCert*, const byte*, wolf_word32, void*);
WOLFSSL_ASN_API void FreeDecodedCert(DecodedCert*);
WOLFSSL_ASN_API int  ParseCert(DecodedCert*, int type, int verify, void* cm);

WOLFSSL_LOCAL int DecodePolicyOID(char *o, wolf_word32 oSz,
                                  const byte *in, wolf_word32 inSz);
WOLFSSL_API int CheckCertSignature(const byte*,wolf_word32,void*,void* cm);
WOLFSSL_LOCAL int ParseCertRelative(DecodedCert*,int type,int verify,void* cm);
WOLFSSL_LOCAL int DecodeToKey(DecodedCert*, int verify);

WOLFSSL_LOCAL const byte* OidFromId(wolf_word32 id, wolf_word32 type, wolf_word32* oidSz);
...
WOLFSSL_ASN_API int ToTraditional(byte* buffer, wolf_word32 length);
WOLFSSL_ASN_API int ToTraditional_ex(byte* buffer, wolf_word32 length,
                                     wolf_word32* algId);
WOLFSSL_LOCAL int ToTraditionalInline(const byte* input, wolf_word32* inOutIdx,
                                      wolf_word32 length);
WOLFSSL_LOCAL int ToTraditionalInline_ex(const byte* input, wolf_word32* inOutIdx,
                                         wolf_word32 length, wolf_word32* algId);
WOLFSSL_LOCAL int ToTraditionalEnc(byte* buffer, wolf_word32 length,const char*,int,
                                   wolf_word32* algId);
WOLFSSL_ASN_API int UnTraditionalEnc(byte* key, wolf_word32 keySz, byte* out,
        wolf_word32* outSz, const char* password, int passwordSz, int vPKCS,
        int vAlgo, byte* salt, wolf_word32 saltSz, int itt, WC_RNG* rng, void* heap);
WOLFSSL_ASN_API int TraditionalEnc(byte* key, wolf_word32 keySz, byte* out,
        wolf_word32* outSz, const char* password, int passwordSz, int vPKCS,
        int vAlgo, int encAlgId, byte* salt, wolf_word32 saltSz, int itt,
        WC_RNG* rng, void* heap);
WOLFSSL_LOCAL int DecryptContent(byte* input, wolf_word32 sz,const char* psw,int pswSz);
WOLFSSL_LOCAL int EncryptContent(byte* input, wolf_word32 sz, byte* out, wolf_word32* outSz,
        const char* password,int passwordSz, int vPKCS, int vAlgo,
        byte* salt, wolf_word32 saltSz, int itt, WC_RNG* rng, void* heap);
WOLFSSL_LOCAL int wc_GetKeyOID(byte* key, wolf_word32 keySz, const byte** curveOID,
        wolf_word32* oidSz, int* algoID, void* heap);
...
WOLFSSL_LOCAL int GetAsnTimeString(void* currTime, byte* buf, wolf_word32 len);
#endif
...
#ifdef WOLFSSL_CERT_GEN
WOLFSSL_ASN_API int SetName(byte* output, wolf_word32 outputSz, CertName* name);
#endif
WOLFSSL_LOCAL int GetShortInt(const byte* input, wolf_word32* inOutIdx, int* number,
                              wolf_word32 maxIdx);
...
WOLFSSL_LOCAL int GetInt(mp_int* mpi, const byte* input, wolf_word32* inOutIdx,
                        wolf_word32 maxIdx);
#ifdef HAVE_OID_ENCODING
    WOLFSSL_LOCAL int EncodeObjectId(const word16* in, wolf_word32 inSz,
        byte* out, wolf_word32* outSz);
#endif
#ifdef HAVE_OID_DECODING
    WOLFSSL_LOCAL int DecodeObjectId(const byte* in, wolf_word32 inSz,
        word16* out, wolf_word32* outSz);
#endif
WOLFSSL_LOCAL int GetASNObjectId(const byte* input, wolf_word32* inOutIdx, int* len,
                                 wolf_word32 maxIdx);
WOLFSSL_LOCAL int SetObjectId(int len, byte* output);
WOLFSSL_LOCAL int GetObjectId(const byte* input, wolf_word32* inOutIdx, wolf_word32* oid,
...
WOLFSSL_LOCAL int GetNameHash(const byte* source, wolf_word32* idx, byte* hash,
                             int maxIdx);
WOLFSSL_LOCAL int wc_CheckPrivateKey(byte* key, wolf_word32 keySz, DecodedCert* der);
WOLFSSL_LOCAL int RsaPublicKeyDerSize(RsaKey* key, int with_header);

#ifdef HAVE_ECC
    /* ASN sig helpers */
    WOLFSSL_LOCAL int StoreECC_DSA_Sig(byte* out, wolf_word32* outLen, mp_int* r,
                                      mp_int* s);
    WOLFSSL_LOCAL int DecodeECC_DSA_Sig(const byte* sig, wolf_word32 sigLen,
                                       mp_int* r, mp_int* s);
#endif
...
WOLFSSL_LOCAL int PemToDer(const unsigned char* buff, long sz, int type,
                          ...
                          int* eccKey);
WOLFSSL_LOCAL int AllocDer(DerBuffer** der, wolf_word32 length, int type, void* heap);
...
struct OcspResponse {
    ...
    wolf_word32  responseSz;      /* length of the OCSP Response */
    ...
    wolf_word32  sigSz;           /* Length in octets for the sig */
    wolf_word32  sigOID;          /* OID for hash used for sig */
...
WOLFSSL_LOCAL void InitOcspResponse(OcspResponse*, CertStatus*, byte*, wolf_word32);
...
WOLFSSL_LOCAL int    EncodeOcspRequest(OcspRequest*, byte*, wolf_word32);
WOLFSSL_LOCAL wolf_word32 EncodeOcspRequestExtensions(OcspRequest*, byte*, wolf_word32);
...
WOLFSSL_LOCAL int VerifyCRL_Signature(SignatureCtx* sigCtx,
                                      const byte* toBeSigned, wolf_word32 tbsSz,
                                      const byte* signature, wolf_word32 sigSz,
                                      wolf_word32 signatureOID, Signer *ca,
                                      void* heap);
WOLFSSL_LOCAL int  ParseCRL(DecodedCRL*, const byte* buff, wolf_word32 sz, void* cm);
