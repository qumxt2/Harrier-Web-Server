...
typedef struct Hmac {
    Hash    hash;
    wolf_word32  ipad[WC_HMAC_BLOCK_SIZE  / sizeof(wolf_word32)];  /* same block size all*/
    wolf_word32  opad[WC_HMAC_BLOCK_SIZE  / sizeof(wolf_word32)];
    wolf_word32  innerHash[WC_MAX_DIGEST_SIZE / sizeof(wolf_word32)];
    void*   heap;                 /* heap hint */
...
/* does init */
WOLFSSL_API int wc_HmacSetKey(Hmac*, int type, const byte* key, wolf_word32 keySz);
WOLFSSL_API int wc_HmacUpdate(Hmac*, const byte*, wolf_word32);
...
WOLFSSL_API int wc_HKDF_Extract(int type, const byte* salt, wolf_word32 saltSz,
                                const byte* inKey, wolf_word32 inKeySz, byte* out);
WOLFSSL_API int wc_HKDF_Expand(int type, const byte* inKey, wolf_word32 inKeySz,
                               const byte* info, wolf_word32 infoSz,
                               byte* out,        wolf_word32 outSz);

WOLFSSL_API int wc_HKDF(int type, const byte* inKey, wolf_word32 inKeySz,
