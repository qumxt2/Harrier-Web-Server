...
#define CHACHA_IV_BYTES    (CHACHA_IV_WORDS * sizeof(wolf_word32))

/* Size of ChaCha chunks */
#define CHACHA_CHUNK_WORDS 16
#define CHACHA_CHUNK_BYTES (CHACHA_CHUNK_WORDS * sizeof(wolf_word32))
...
typedef struct ChaCha {
    wolf_word32 X[CHACHA_CHUNK_WORDS];           /* state of cipher */
...
/**
  ...
  */
WOLFSSL_API int wc_Chacha_SetIV(ChaCha* ctx, const byte* inIv, wolf_word32 counter);

WOLFSSL_API int wc_Chacha_Process(ChaCha* ctx, byte* cipher, const byte* plain,
                              wolf_word32 msglen);
WOLFSSL_API int wc_Chacha_SetKey(ChaCha* ctx, const byte* key, wolf_word32 keySz);
