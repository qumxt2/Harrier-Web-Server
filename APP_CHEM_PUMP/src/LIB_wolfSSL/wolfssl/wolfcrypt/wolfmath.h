...
#define __WOLFMATH_H__
    ...
    WOLFSSL_API int wc_export_int(mp_int* mp, byte* buf, wolf_word32* len, 
        wolf_word32 keySz, int encType);

    #ifdef HAVE_WOLF_BIGINT
        void wc_bigint_init(WC_BIGINT* a);
        int wc_bigint_alloc(WC_BIGINT* a, wolf_word32 sz);
        int wc_bigint_from_unsigned_bin(WC_BIGINT* a, const byte* in, wolf_word32 inlen);
        int wc_bigint_to_unsigned_bin(WC_BIGINT* a, byte* out, wolf_word32* outlen);
        void wc_bigint_zero(WC_BIGINT* a);
        void wc_bigint_free(WC_BIGINT* a);

        int wc_mp_to_bigint(mp_int* src, WC_BIGINT* dst);
        int wc_mp_to_bigint_sz(mp_int* src, WC_BIGINT* dst, wolf_word32 sz);
